export interface IsoBusStopOptions {
  accent?: string;
}
...
export function createIsoBusStop(options: IsoBusStopOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(4076762696);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="120" ry="30" fill="#000" opacity="0.3"/>');
parts.push(box(-95, 30, 0, 190, 22, 5, '#27272a'));
for (let i = 0; i < 8; i++) parts.push(box(-88 + i * 23, 36, 5, 12, 4, 1.5, '#fbbf24'));
parts.push(box(-62, -12, 0, 124, 44, 6, '#3f3f46'));
parts.push(box(-58, -14, 6, 108, 3, 56, '#0e7490'));
parts.push(box(-58, -14, 62, 116, 48, 8, '#18181b'));
parts.push(box(-58, -14, 70, 116, 48, 3, shade(accent, 0.8)));
parts.push(box(-54, 8, 6, 8, 8, 52, '#52525b'));
parts.push(box(44, 8, 6, 8, 8, 52, '#52525b'));
parts.push(box(-44, -4, 6, 76, 16, 8, '#7f1d1d'));
parts.push(box(-44, -4, 14, 76, 16, 5, '#991b1b'));
parts.push(box(72, 4, 0, 5, 5, 64, '#52525b'));
parts.push(box(66, 0, 64, 18, 14, 20, '#18181b'));
parts.push(led(75, 7, 74, accent, 1.6, 0));
parts.push(led(75, 7, 68, '#f472b6', 1.6, 0.8));
