export interface IsoSmartSpeakerOptions {
  accent?: string;
}
...
export function createIsoSmartSpeaker(options: IsoSmartSpeakerOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(2099244494);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="70" ry="18" fill="#000" opacity="0.35"/>');
parts.push(box(-64, 14, 0, 128, 24, 4, '#3f3f46'));
parts.push(cyl(0, 0, 4, 26, 44, '#44403c', 12));
for (let i = 0; i < 3; i++) parts.push('<path d="' + poly([[-24, 0, 12 + i * 13], [24, 0, 12 + i * 13]]) + '" stroke="#57534e" stroke-width="1.5" fill="none"/>');
parts.push('<ellipse cx="' + project(0, 0, 48)[0].toFixed(1) + '" cy="' + project(0, 0, 48)[1].toFixed(1) + '" rx="25" ry="12" fill="#18181b"/>');
parts.push('<ellipse cx="' + project(0, 0, 49)[0].toFixed(1) + '" cy="' + project(0, 0, 49)[1].toFixed(1) + '" rx="21" ry="10" fill="none" stroke="' + accent + '" stroke-width="2"><animate attributeName="opacity" values="1;0.3;1" dur="1.8s" repeatCount="indefinite"/></ellipse>');
for (let i = 0; i < 4; i++) {
  const a = i / 4 * Math.PI * 2;
  parts.push('<circle cx="' + (project(Math.cos(a) * 14, Math.sin(a) * 14, 49)[0]).toFixed(1) + '" cy="' + project(Math.cos(a) * 14, Math.sin(a) * 14, 49)[1].toFixed(1) + '" r="2.2" fill="' + accent + '"><animate attributeName="opacity" values="1;0.2;1" dur="1.8s" begin="' + (i * 0.45) + 's" repeatCount="indefinite"/></circle>');
}
for (let i = 0; i < 3; i++) {
  parts.push('<ellipse cx="' + (60 + i * 18) + '" cy="' + (-60 - i * 16) + '" rx="' + (10 + i * 7) + '" ry="' + (5 + i * 3) + '" fill="none" stroke="#67e8f9" stroke-width="1.5" opacity="0"><animate attributeName="opacity" values="0;0.7;0" dur="2.4s" begin="' + (i * 0.4) + 's" repeatCount="indefinite"/></ellipse>');
}
parts.push(box(-78, 20, 4, 22, 14, 34, '#18181b'));
parts.push('<path d="' + poly([[-74, 20.5, 10], [-60, 20.5, 10], [-60, 20.5, 30], [-74, 20.5, 30]]) + '" fill="#0ea5e9" opacity="0.7"/>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0f1418"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
