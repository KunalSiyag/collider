export interface IsoOilPumpjackOptions {
  accent?: string;
}
...
export function createIsoOilPumpjack(options: IsoOilPumpjackOptions = {}): string {
  const { accent = '#f59e0b' } = options;
  const rand = rng(3862947971);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="120" ry="30" fill="#292524" opacity="0.7"/>');
parts.push(box(-70, -26, 0, 140, 52, 8, '#57534e'));
parts.push('<path d="M-30 8 L-46 -84 M-30 8 L-14 -84 M-46 -84 L-14 -84" stroke="#78716c" stroke-width="5" fill="none"/>');
const piv = project(-30, 8, -84);
let beam = box(-88, 2, -90, 116, 10, 8, '#f59e0b');
beam += box(-96, 0, -98, 14, 14, 16, '#44403c');
beam += orb(20, 6, -80, 10, '#3f3f46');
parts.push('<g><animateTransform attributeName="transform" type="rotate" values="-7 ' + piv[0].toFixed(0) + ' ' + piv[1].toFixed(0) + ';7 ' + piv[0].toFixed(0) + ' ' + piv[1].toFixed(0) + ';-7 ' + piv[0].toFixed(0) + ' ' + piv[1].toFixed(0) + '" dur="3.4s" repeatCount="indefinite"/>' + beam + '</g>');
parts.push('<path d="M' + piv[0].toFixed(1) + ' ' + piv[1].toFixed(1) + ' L' + piv[0].toFixed(1) + ' ' + (piv[1] + 90).toFixed(1) + '" stroke="#78716c" stroke-width="4" fill="none"/>');
parts.push(cyl(52, 20, 8, 12, 34, '#71717a', 8));
parts.push(box(44, 12, 42, 18, 18, 10, '#44403c'));
parts.push('<circle cx="' + project(53, 21, 54)[0].toFixed(1) + '" cy="' + project(53, 21, 54)[1].toFixed(1) + '" r="3" fill="#0a0a0a"><animate attributeName="cy" values="' + project(53, 21, 54)[1].toFixed(0) + ';' + (project(53, 21, 54)[1] + 26).toFixed(0) + '" dur="1.1s" repeatCount="indefinite"/></circle>');
parts.push(cyl(-104, 30, 8, 13, 22, '#3f3f46', 10));
parts.push('<ellipse cx="-104" cy="30" rx="12" ry="6" fill="#0a0a0a"/>');
parts.push(led(-30, 4, -98, '#ef4444', 1.6, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#170f0d"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
