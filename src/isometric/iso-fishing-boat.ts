export interface IsoFishingBoatOptions {
  accent?: string;
}
...
export function createIsoFishingBoat(options: IsoFishingBoatOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(1249032602);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="40" rx="125" ry="34" fill="#164e63"/>');
parts.push('<path d="M-110 30 Q-60 44 0 36 T110 32" stroke="#22d3ee" stroke-width="2" fill="none" opacity="0.4"><animate attributeName="opacity" values="0.4;0.15;0.4" dur="3s" repeatCount="indefinite"/></path>');
parts.push(box(-56, -24, 10, 96, 48, 26, '#7c4a21'));
parts.push(pyr(40, -24, 10, 30, 48, 24, '#8d5a2b'));
parts.push(box(-56, -24, 36, 100, 52, 6, '#a16207'));
parts.push(box(-40, -18, 42, 40, 34, 30, '#e7e5e4'));
parts.push('<path d="' + poly([[-34, 16, 50], [-10, 16, 50], [-10, 16, 64], [-34, 16, 64]]) + '" fill="#bae6fd"/>');
parts.push('<path d="M20 -2 L20 -96 M-4 -88 L44 -88" stroke="#78350f" stroke-width="4" fill="none"/>');
parts.push('<path d="M-4 -88 L-30 -60 M-4 -88 Q-20 -70 -30 -60" stroke="#e7e5e4" stroke-width="1.5" fill="none" opacity="0.7"/>');
parts.push(orb(-64, 20, 42, 6, accent));
parts.push(orb(-74, 26, 42, 5, '#fbbf24'));
parts.push(led(24, 4, -60 + 102, '#fbbf24', 2, 0));
parts.push('<circle cx="90" cy="30" r="3" fill="#e7e5e4" opacity="0.5"><animate attributeName="cx" values="90;120;90" dur="4s" repeatCount="indefinite"/></circle>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0b1220"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
