export interface IsoSaunaCabinOptions {
  accent?: string;
}
...
export function createIsoSaunaCabin(options: IsoSaunaCabinOptions = {}): string {
  const { accent = '#f59e0b' } = options;
  const rand = rng(2123424365);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="76" rx="115" ry="28" fill="#14532d" opacity="0.35"/>');
function pine(x, y, s) {
  return pyr(x, y, 30 * s + 10, 26 * s, 22 * s, 34 * s, '#166534') + pyr(x + 3 * s, y + 2 * s, 52 * s + 10, 20 * s, 16 * s, 26 * s, '#15803d');
}
parts.push(pine(-92, -6, 1.4));
parts.push(pine(84, 16, 1.1));
parts.push(box(-48, -28, 0, 96, 56, 58, '#8d5a2b'));
for (let i = 0; i < 5; i++) parts.push('<path d="' + poly([[-48, 28, 8 + i * 11], [48, 28, 8 + i * 11]]) + '" stroke="#713f12" stroke-width="2" fill="none"/>');
parts.push(box(-54, -34, 58, 108, 68, 9, '#44403c'));
parts.push('<path d="' + poly([[-36, 28.5, 14], [-12, 28.5, 14], [-12, 28.5, 50], [-36, 28.5, 50]]) + '" fill="#292524"/>');
parts.push('<path d="' + poly([[8, 28.5, 24], [32, 28.5, 24], [32, 28.5, 46], [8, 28.5, 46]]) + '" fill="' + accent + '"><animate attributeName="opacity" values="1;0.65;1" dur="2.4s" repeatCount="indefinite"/></path>');
parts.push(box(-38, -24, 67, 14, 14, 26, '#57534e'));
parts.push(puff(-31, -17, 96, 8, 3, 0));
parts.push(puff(-29, -19, 104, 11, 3.6, 1.3));
parts.push(cyl(66, 26, 0, 12, 14, '#b45309', 10));
parts.push('<ellipse cx="' + project(66, 26, 15)[0].toFixed(1) + '" cy="' + project(66, 26, 15)[1].toFixed(1) + '" rx="10" ry="5" fill="#7dd3fc"/>');
parts.push('<path d="M' + project(70, 30, 18)[0].toFixed(1) + ' ' + project(70, 30, 18)[1].toFixed(1) + ' L' + project(78, 34, 26)[0].toFixed(1) + ' ' + project(78, 34, 26)[1].toFixed(1) + '" stroke="#a16207" stroke-width="3" stroke-linecap="round" fill="none"/>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0f1210"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
