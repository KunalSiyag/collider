export interface IsoSolarArrayOptions {
  accent?: string;
}
...
export function createIsoSolarArray(options: IsoSolarArrayOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(2873766510);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="125" ry="30" fill="#3f6212" opacity="0.35"/>');
function panel(px, py) {
  let s = box(px, py, 10, 44, 30, 5, '#1e3a8a');
  for (let i = 0; i < 3; i++) s += '<path d="' + poly([[px + 6 + i * 14, py + 2, 15.5], [px + 6 + i * 14, py + 28, 15.5]]) + '" stroke="#3b82f6" stroke-width="1.5" fill="none"/>';
  s += '<path d="' + poly([[px + (py > 0 ? 34 : 34), py - 12, 0], [px + 38, py - 12, 0], [px + 38, py - 8, 0], [px + 34, py - 8, 0]]) + '" fill="#52525b"/>';
  return s;
}
for (let r = 0; r < 3; r++) {
  for (let c = 0; c < 2; c++) parts.push(panel(-56 + c * 60, -40 + r * 40));
}
parts.push(orb(96, -80, 120, 13, accent));
for (let i = 0; i < 8; i++) {
  const ra = i / 8 * Math.PI * 2;
  parts.push('<path d="M' + project(96, -80, 120)[0].toFixed(1) + ' ' + project(96, -80, 120)[1].toFixed(1) + ' l' + (Math.cos(ra) * 22).toFixed(1) + ' ' + (Math.sin(ra) * 11).toFixed(1) + '" stroke="' + accent + '" stroke-width="2"><animate attributeName="opacity" values="0.9;0.3;0.9" dur="2.6s" begin="' + (i * 0.2) + 's" repeatCount="indefinite"/></path>');
}
parts.push(box(74, 30, 0, 18, 14, 12, '#3f3f46'));
parts.push(led(83, 37, 14, '#4ade80', 1.8, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#12140c"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
