export interface IsoBbqGrillOptions {
  accent?: string;
}
...
export function createIsoBbqGrill(options: IsoBbqGrillOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(3278116021);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="90" ry="24" fill="#000" opacity="0.35"/>');
parts.push('<path d="M-24 40 L-40 84 M24 40 L40 84 M0 46 L0 84" stroke="#3f3f46" stroke-width="4" stroke-linecap="round" fill="none"/>');
parts.push(cyl(0, 0, 40, 36, 18, '#27272a', 12));
const grate = project(0, 0, 58);
parts.push('<ellipse cx="' + grate[0].toFixed(1) + '" cy="' + grate[1].toFixed(1) + '" rx="31" ry="15" fill="#52525b"/>');
for (let i = -2; i <= 2; i++) parts.push('<ellipse cx="' + grate[0].toFixed(1) + '" cy="' + grate[1].toFixed(1) + '" rx="' + (31 - Math.abs(i) * 2).toFixed(0) + '" ry="' + (15 - Math.abs(i)).toFixed(0) + '" fill="none" stroke="#a8a29e" stroke-width="1" transform="translate(0 ' + i * 4.5 + ')"/>');
for (let i = 0; i < 3; i++) parts.push(box(-18 + i * 14, -4, 59, 10, 7, 4, '#b45309'));
parts.push('<circle cx="-58" cy="-30" r="22" fill="#7f1d1d"/><circle cx="-58" cy="-30" r="19" fill="#991b1b"/>');
parts.push(orb(-58, -30, -14, 5, '#27272a'));
parts.push(puff(0, 0, 66, 7, 2.6, 0));
parts.push(puff(2, 2, 74, 9, 3.2, 1.2));
parts.push('<circle cx="20" cy="-70" r="2" fill="#fbbf24"><animate attributeName="cy" values="-70;-96;-70" dur="1.4s" repeatCount="indefinite"/><animate attributeName="opacity" values="1;0;1" dur="1.4s" repeatCount="indefinite"/></circle>');
parts.push(cyl(66, 26, 84, 5, 16, '#dc2626', 8));
parts.push(cyl(74, 20, 84, 5, 16, '#fbbf24', 8));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#131010"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
