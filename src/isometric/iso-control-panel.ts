export interface IsoControlPanelOptions {
  accent?: string;
}
...
export function createIsoControlPanel(options: IsoControlPanelOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(3468470461);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="80" rx="105" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-52, -24, 0, 104, 48, 56, '#3f3f46'));
parts.push('<path d="' + poly([[-52, 24, 8], [-44, 24, 8], [-44, 24, 14], [-52, 24, 14]]) + '" fill="' + accent + '"/>');
parts.push('<path d="' + poly([[-36, 24, 8], [-28, 24, 8], [-28, 24, 14], [-36, 24, 14]]) + '" fill="#18181b"/>');
parts.push(box(-56, -28, 56, 112, 56, 12, '#52525b'));
for (let r = 0; r < 2; r++) for (let c = 0; c < 6; c++) {
  parts.push(box(-48 + c * 17, -22 + r * 20, 68, 10, 10, 5, ['#4ade80', '#ef4444', accent][(r + c) % 3]));
}
parts.push('<path d="' + poly([[-50, -26, 62], [50, -26, 62], [50, -26, 66], [-50, -26, 66]]) + '" fill="#020617"/>');
for (let i = 0; i < 5; i++) parts.push(led(-40 + i * 20, -25, 64, i % 2 ? '#4ade80' : '#22d3ee', 0.9, i * 0.18));
const lv = project(58, 16, 66);
parts.push(cyl(58, 16, 56, 9, 10, '#27272a', 8));
parts.push('<g><animateTransform attributeName="transform" type="rotate" values="-24 ' + lv[0].toFixed(0) + ' ' + lv[1].toFixed(0) + ';24 ' + lv[0].toFixed(0) + ' ' + lv[1].toFixed(0) + ';-24 ' + lv[0].toFixed(0) + ' ' + lv[1].toFixed(0) + '" dur="4s" repeatCount="indefinite"/><path d="M' + lv[0].toFixed(1) + ' ' + lv[1].toFixed(1) + ' L' + lv[0].toFixed(1) + ' ' + (lv[1] - 34).toFixed(1) + '" stroke="#a1a1aa" stroke-width="5" stroke-linecap="round"/><circle cx="' + lv[0].toFixed(1) + '" cy="' + (lv[1] - 34).toFixed(1) + '" r="7" fill="#ef4444"/></g>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#131108"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
