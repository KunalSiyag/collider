export interface IsoPancakeStackOptions {
  accent?: string;
}
...
export function createIsoPancakeStack(options: IsoPancakeStackOptions = {}): string {
  const { accent = '#f59e0b' } = options;
  const rand = rng(413266749);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="80" ry="20" fill="#000" opacity="0.35"/>');
parts.push(cyl(0, 0, 0, 52, 6, '#f5f5f4', 12));
for (let i = 0; i < 5; i++) {
  const off = i % 2 ? 3 : -3;
  parts.push(cyl(off, off * 0.6, 6 + i * 11, 38 - i, 11, ['#d97706', '#c2410c'][i % 2], 12));
}
parts.push('<ellipse cx="0" cy="' + (project(0, 0, 61)[1]).toFixed(1) + '" rx="34" ry="16" fill="#b45309" opacity="0.9"/>');
parts.push('<ellipse cx="-8" cy="' + (project(0, 0, 61)[1] - 3).toFixed(1) + '" rx="10" ry="5" fill="#d97706" opacity="0.8"/>');
parts.push(box(-7, -5, 61, 14, 12, 9, '#fef9c3'));
parts.push(orb(12, 8, 64, 4.5, '#be123c'));
parts.push(orb(18, 2, 64, 4, '#be123c'));
parts.push('<path d="M' + project(-30, 20, 20)[0].toFixed(1) + ' ' + project(-30, 20, 20)[1].toFixed(1) + ' q-6 14 4 22" stroke="#b45309" stroke-width="3" fill="none"/>');
parts.push(orb(-30, 20, 22, 2.5, '#b45309'));
parts.push(puff(0, 0, 74, 6, 3, 0.4, '#fef3c7'));
