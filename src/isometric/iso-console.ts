export interface IsoConsoleOptions {
  accent?: string;
}
...
export function createIsoConsole(options: IsoConsoleOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(3679575502);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="115" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-70, -26, 0, 140, 52, 26, '#27272a'));
parts.push(box(-70, -26, 26, 140, 52, 10, '#3f3f46'));
for (let ch = 0; ch < 8; ch++) {
  const x = -62 + ch * 17;
  parts.push('<path d="' + poly([[x, 20, 30], [x + 8, 20, 30]]) + '" stroke="#71717a" stroke-width="3" fill="none"/>');
  const fy = 34 + rand() * 12;
  parts.push('<path d="' + poly([[x - 1, 20.5, fy], [x + 9, 20.5, fy], [x + 9, 20.5, fy + 4], [x - 1, 20.5, fy + 4]]) + '" fill="' + accent + '"/>');
}
for (let k = 0; k < 6; k++) parts.push(orb(-60 + k * 24, -18, 37, 4, '#a1a1aa'));
for (let m = 0; m < 4; m++) {
  const mx = -52 + m * 30;
  parts.push('<path d="' + poly([[mx, -22, 31], [mx + 16, -22, 31], [mx + 16, -22, 33], [mx, -22, 33]]) + '" fill="#020617"/>');
  const hgt = 6 + rand() * 10;
  parts.push('<path d="' + poly([[mx + 2, -22.2, 32], [mx + 6, -22.2, 32], [mx + 6, -22.2, 32 + hgt], [mx + 2, -22.2, 32 + hgt]]) + '" fill="#4ade80"><animate attributeName="opacity" values="1;0.4;' + (0.4 + rand()).toFixed(1).slice(0, 3) + ';1" dur="0.8s" repeatCount="indefinite"/></path>');
}
parts.push(box(58, 6, 0, 26, 20, 14, '#18181b'));
parts.push(led(64, 10, 15, accent, 1.6, 0));
parts.push(led(74, 10, 15, '#f472b6', 1.6, 0.8));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0f1318"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
