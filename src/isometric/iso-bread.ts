export interface IsoBreadOptions {
  accent?: string;
}
...
export function createIsoBread(options: IsoBreadOptions = {}): string {
  const { accent = '#d97706' } = options;
  const rand = rng(3021898145);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="50" rx="92" ry="24" fill="#000" opacity="0.35"/>');
parts.push(box(-52, -28, 0, 104, 56, 9, '#a16207'));
parts.push(box(-48, -24, 9, 96, 48, 3, '#b45309'));
parts.push('<ellipse cx="-4" cy="6" rx="40" ry="16" fill="#fefce8" opacity="0.18"/>');
parts.push(box(-42, -18, 12, 44, 24, 17, '#c98a3d'));
parts.push(box(-46, -20, 12, 5, 26, 15, '#b97a34'));
parts.push(box(2, -18, 12, 5, 26, 15, '#b97a34'));
for (let i = 0; i < 3; i++) parts.push(box(-32 + i * 12, -14, 29, 3, 18, 2, '#8a5a22'));
parts.push(box(16, -2, 12, 38, 9, 8, '#d19a4a'));
parts.push(cyl(44, 18, 12, 7, 14, '#78716c', 8));
parts.push('<path d="M44 30 L44 6 M40 30 L38 8 M48 30 L50 8" stroke="#a3e635" stroke-width="2" fill="none"/>');
parts.push(puff(-20, -6, 32, 7, 3, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#171208"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
