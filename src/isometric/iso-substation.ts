export interface IsoSubstationOptions {
  accent?: string;
}
...
export function createIsoSubstation(options: IsoSubstationOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(319331967);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="76" rx="120" ry="30" fill="#000" opacity="0.35"/>');
parts.push(box(-70, -40, 0, 140, 80, 5, '#44403c'));
for (let i = 0; i < 6; i++) parts.push(box(-66 + i * 26, -38, 5, 4, 4, 34, '#78716c'));
for (let i = 0; i < 5; i++) parts.push(box(-66, -36 + i * 19, 5, 132, 3, 2, '#78716c'));
parts.push(box(-34, -22, 5, 68, 44, 46, '#52525b'));
for (let i = 0; i < 6; i++) parts.push(box(-30 + i * 11, -24, 51, 6, 40, 8, '#3f3f46'));
for (const tx of [-16, 20]) {
  for (let s = 0; s < 3; s++) parts.push(cyl(tx, -30, 59 + s * 9, 5, 7, '#a8a29e', 8));
}
parts.push(box(-52, -34, 88, 104, 8, 6, '#78716c'));
parts.push(box(-52, -34, 94, 6, 6, 6, '#78716c'));
parts.push(box(46, -34, 94, 6, 6, 6, '#78716c'));
parts.push('<path d="M' + project(-16, -30, 86)[0].toFixed(1) + ' ' + project(-16, -30, 86)[1].toFixed(1) + ' Q' + project(2, -30, 98)[0].toFixed(1) + ' ' + project(2, -30, 102)[1].toFixed(1) + ' ' + project(20, -30, 86)[0].toFixed(1) + ' ' + project(20, -30, 86)[1].toFixed(1) + '" stroke="#18181b" stroke-width="2" fill="none"/>');
parts.push(pyr(52, 16, 5, 20, 18, 16, accent));
parts.push('<path d="M' + project(57, 21, 21)[0].toFixed(1) + ' ' + project(57, 21, 21)[1].toFixed(1) + ' L' + project(67, 21, 13)[0].toFixed(1) + ' ' + project(67, 21, 13)[1].toFixed(1) + '" stroke="#1c1917" stroke-width="3"/>');
parts.push(led(-40, 20, 54, accent, 2.2, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#111310"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
