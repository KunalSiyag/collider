export interface IsoTacoTruckOptions {
  accent?: string;
}
...
export function createIsoTacoTruck(options: IsoTacoTruckOptions = {}): string {
  const { accent = '#4ade80' } = options;
  const rand = rng(4007636734);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="64" rx="115" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-62, -28, 12, 116, 56, 48, accent));
parts.push('<path d="' + poly([[-62, 28, 34], [54, 28, 34], [54, 28, 44], [-62, 28, 44]]) + '" fill="#ef4444"/>');
parts.push('<path d="' + poly([[-16, 28.5, 20], [26, 28.5, 20], [26, 28.5, 52], [-16, 28.5, 52]]) + '" fill="#1c1917"/>');
parts.push(box(-18, 26, 16, 48, 8, 4, '#a16207'));
parts.push(box(-62, -28, 60, 124, 60, 6, '#b45309'));
parts.push('<path d="' + poly([[-40, 28, 60], [40, 28, 60], [40, 28, 74], [-40, 28, 74]]) + '" fill="#dc2626"/>');
for (let i = 0; i < 4; i++) parts.push('<path d="' + poly([[-36 + i * 20, 28.2, 63], [-24 + i * 20, 28.2, 63], [-24 + i * 20, 28.2, 71], [-36 + i * 20, 28.2, 71]]) + '" fill="#fef3c7"/>');
for (let i = 0; i < 8; i++) {
  parts.push(orb(-56 + i * 16, 29, 79, 2.6, ['#fbbf24', '#f472b6', '#22d3ee'][i % 3]));
}
parts.push('<circle cx="' + project(-40, 28, 12)[0].toFixed(1) + '" cy="' + project(-40, 28, 12)[1].toFixed(1) + '" r="12" fill="#18181b"/><circle cx="' + project(-40, 28, 12)[0].toFixed(1) + '" cy="' + project(-40, 28, 12)[1].toFixed(1) + '" r="5" fill="#52525b"/>');
parts.push('<circle cx="' + project(36, 28, 12)[0].toFixed(1) + '" cy="' + project(36, 28, 12)[1].toFixed(1) + '" r="12" fill="#18181b"/><circle cx="' + project(36, 28, 12)[0].toFixed(1) + '" cy="' + project(36, 28, 12)[1].toFixed(1) + '" r="5" fill="#52525b"/>');
for (let i = 0; i < 3; i++) parts.push(box(70, -20 + i * 14, 12, 22, 16, 12, i % 2 ? '#ef4444' : '#f59e0b'));
parts.push(led(60, -24, 66, '#fef08a', 2, 0));
