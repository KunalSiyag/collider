export interface IsoBonsaiOptions {
  accent?: string;
}
...
export function createIsoBonsai(options: IsoBonsaiOptions = {}): string {
  const { accent = '#22c55e' } = options;
  const rand = rng(912547223);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="40" rx="70" ry="18" fill="#000" opacity="0.3"/>');
parts.push(box(-36, -24, 0, 72, 48, 10, '#5d4037'));
parts.push(box(-38, -26, 10, 76, 52, 4, '#6d4c41'));
parts.push(box(-32, -20, 14, 64, 40, 4, '#3e2723'));
parts.push(box(-4, -4, 18, 10, 8, 16, '#5d4037'));
parts.push(box(-10, 0, 34, 10, 8, 14, '#4e342e'));
parts.push(box(-2, -8, 48, 8, 8, 12, '#5d4037'));
parts.push(orb(-12, -8, 62, 17, '#166534'));
parts.push(orb(10, 4, 68, 14, '#15803d'));
parts.push(orb(-2, 12, 76, 11, accent));
parts.push(orb(4, 16, 78, 3, '#fb7185'));
parts.push(orb(-8, 20, 74, 2.6, '#fb7185'));
for (let i = 0; i < 5; i++) parts.push(orb(-24 + i * 12, 14, 19, 2, '#78716c'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#101710"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
