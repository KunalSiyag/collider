export interface IsoCamperVanOptions {
  accent?: string;
}
...
export function createIsoCamperVan(options: IsoCamperVanOptions = {}): string {
  const { accent = '#f59e0b' } = options;
  const rand = rng(1860805823);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="66" rx="110" ry="24" fill="#000" opacity="0.35"/>');
parts.push(box(-62, -28, 12, 114, 56, 42, '#e7e5e4'));
parts.push(box(52, -28, 12, 26, 56, 30, '#d6d3d1'));
parts.push('<path d="' + poly([[-62, 28, 26], [52, 28, 26], [52, 28, 36], [-62, 28, 36]]) + '" fill="' + accent + '"/>');
for (let i = 0; i < 4; i++) parts.push('<path d="' + poly([[-52 + i * 26, 28.2, 40], [-34 + i * 26, 28.2, 40], [-34 + i * 26, 28.2, 52], [-52 + i * 26, 28.2, 52]]) + '" fill="#0c4a6e"/>');
parts.push('<path d="' + poly([[78, -28, 26], [78, 6, 26], [78, 6, 38], [78, -28, 38]]) + '" fill="#bae6fd"/>');
const w1 = project(-40, 28, 14);
const w2 = project(40, 28, 14);
const w3 = project(78, -6, 14);
parts.push('<circle cx="' + w1[0].toFixed(1) + '" cy="' + w1[1].toFixed(1) + '" r="11" fill="#18181b"/><circle cx="' + w1[0].toFixed(1) + '" cy="' + w1[1].toFixed(1) + '" r="4.5" fill="#52525b"/>');
parts.push('<circle cx="' + w2[0].toFixed(1) + '" cy="' + w2[1].toFixed(1) + '" r="11" fill="#18181b"/><circle cx="' + w2[0].toFixed(1) + '" cy="' + w2[1].toFixed(1) + '" r="4.5" fill="#52525b"/>');
parts.push('<circle cx="' + w3[0].toFixed(1) + '" cy="' + w3[1].toFixed(1) + '" r="11" fill="#18181b"/><circle cx="' + w3[0].toFixed(1) + '" cy="' + w3[1].toFixed(1) + '" r="4.5" fill="#52525b"/>');
parts.push(box(-46, -18, 54, 84, 30, 4, '#52525b'));
parts.push(box(-38, -10, 58, 72, 13, 6, '#0ea5e9'));
parts.push(pyr(34, -10, 58, 10, 13, 7, '#0ea5e9'));
parts.push(led(79, 24, 20, '#fbbf24', 2.4, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#141117"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
