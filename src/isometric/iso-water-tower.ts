export interface IsoWaterTowerOptions {
  accent?: string;
}
...
export function createIsoWaterTower(options: IsoWaterTowerOptions = {}): string {
  const { accent = '#b45309' } = options;
  const rand = rng(3570781776);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="100" rx="90" ry="22" fill="#000" opacity="0.35"/>');
parts.push(box(-40, -30, 0, 80, 60, 8, '#3f3f46'));
for (const [lx, ly] of [[-34, -24], [-34, 20], [26, -24], [26, 20]]) {
  parts.push(box(lx, ly, 8, 7, 7, 96, '#78716c'));
}
parts.push('<path d="M-30 30 L30 70 M-30 70 L30 30 M-30 58 L30 98 M-30 98 L30 58" stroke="#57534e" stroke-width="2.5" fill="none"/>');
parts.push(cyl(0, 0, 104, 42, 52, accent, 12));
for (let i = 0; i < 2; i++) parts.push('<path d="' + poly([[-41, 0, 116 + i * 24], [41, 0, 116 + i * 24]]) + '" stroke="#78350f" stroke-width="4" fill="none"/>');
parts.push(pyr(-44, -44, 156, 88, 88, 26, '#7f1d1d'));
parts.push(led(0, 0, 186, '#ef4444', 1.6, 0));
parts.push('<path d="M-46 -20 L-46 104 M-38 -20 L-38 104 M-46 40 A8 8 0 0 1 -38 48 M-46 72 A8 8 0 0 1 -38 80" stroke="#a1a1aa" stroke-width="2.5" fill="none"/>');
parts.push('<path d="M0 104 L0 10" stroke="#57534e" stroke-width="5" fill="none"/>');
parts.push(box(-6, -6, 4, 12, 12, 10, '#52525b'));
parts.push(orb(14, -50, 168, 3.5, '#57534e'));
parts.push(orb(22, -56, 172, 3, '#78716c'));
