export interface IsoEspressoMachineOptions {
  accent?: string;
}
...
export function createIsoEspressoMachine(options: IsoEspressoMachineOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(195483081);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="66" rx="85" ry="22" fill="#000" opacity="0.35"/>');
parts.push(box(-58, -24, 0, 116, 48, 8, '#7c4a21'));
parts.push(box(-50, -18, 8, 100, 40, 52, '#a8a29e'));
parts.push('<path d="' + poly([[-50, 22, 20], [50, 22, 20], [50, 22, 26], [-50, 22, 26]]) + '" fill="#78716c"/>');
parts.push(box(-50, -18, 60, 100, 40, 10, '#d6d3d1'));
for (let i = 0; i < 4; i++) parts.push(cyl(-38 + i * 25, -8, 70, 9, 8, '#fafaf9', 8));
parts.push(box(-14, 18, 30, 28, 10, 10, '#57534e'));
parts.push(box(-10, 24, 26, 18, 6, 4, '#44403c'));
parts.push(cyl(0, 30, 8, 10, 12, '#fafaf9', 8));
const stream = project(0, 32, 22);
parts.push('<rect x="' + (stream[0] - 1.5).toFixed(1) + '" y="' + stream[1].toFixed(1) + '" width="3" height="10" fill="#78350f"><animate attributeName="height" values="10;14;10" dur="0.8s" repeatCount="indefinite"/></rect>');
const gauge = project(-34, 22, 44);
parts.push('<circle cx="' + gauge[0].toFixed(1) + '" cy="' + gauge[1].toFixed(1) + '" r="7" fill="#fafaf9"/><path d="M' + gauge[0].toFixed(1) + ' ' + gauge[1].toFixed(1) + ' L' + (gauge[0] + 4).toFixed(1) + ' ' + (gauge[1] - 4).toFixed(1) + '" stroke="#ef4444" stroke-width="2"/>');
parts.push(led(40, 20, 52, accent, 1.4, 0));
parts.push(puff(52, -14, 30, 4, 2.2, 0.4));
parts.push(puff(54, -12, 36, 5.5, 2.8, 1.2));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#11141a"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
