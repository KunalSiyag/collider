export interface IsoKiteShackOptions {
  accent?: string;
}
...
export function createIsoKiteShack(options: IsoKiteShackOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(825328163);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="66" rx="115" ry="30" fill="#e7d8b0" opacity="0.14"/>');
parts.push(box(-52, -28, 0, 104, 56, 52, '#a16207'));
for (let i = 0; i < 4; i++) parts.push('<path d="' + poly([[-52, 28, 10 + i * 12], [52, 28, 10 + i * 12]]) + '" stroke="#7c4a21" stroke-width="2" fill="none"/>');
parts.push(box(-58, -34, 52, 116, 68, 8, '#57534e'));
parts.push(box(-16, 28, 8, 26, 4, 34, '#3f3f46'));
parts.push('<path d="' + poly([[-12, 27.8, 12], [6, 27.8, 12], [6, 27.8, 38], [-12, 27.8, 38]]) + '" fill="#fbbf24"/>');
parts.push(box(64, 10, 0, 8, 30, 76, '#0ea5e9'));
parts.push(box(76, 18, 0, 8, 30, 68, '#f472b6'));
parts.push('<g><animateTransform attributeName="transform" type="translate" values="0 0;10 -12;0 0" dur="5s" repeatCount="indefinite"/>');
parts.push('<polygon points="30,-150 52,-128 30,-106 8,-128" fill="' + accent + '"/><polygon points="30,-150 52,-128 30,-128" fill="' + shade(accent, 0.8) + '"/>');
parts.push('<path d="M30 -106 q-8 14 2 26 q8 10 -2 20" stroke="#e2e8f0" stroke-width="1.5" fill="none" stroke-dasharray="4 4"/></g>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#101720"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
