export interface IsoWindTurbineOptions {
  accent?: string;
}
...
export function createIsoWindTurbine(options: IsoWindTurbineOptions = {}): string {
  const { accent = '#e7e5e4' } = options;
  const rand = rng(1520472889);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="110" rx="120" ry="28" fill="#14532d" opacity="0.5"/>');
parts.push(box(-70, 40, 0, 140, 60, 14, '#365314'));
parts.push(cyl(0, 0, 14, 13, 150, '#f4f4f5', 10));
parts.push(cyl(0, 0, 164, 9, 10, '#e4e4e7', 10));
const hub = project(0, 0, 182);
let rotor = '';
rotor += '<circle cx="' + hub[0].toFixed(1) + '" cy="' + hub[1].toFixed(1) + '" r="7" fill="#d4d4d8"/>';
rotor += '<path d="' + poly([[0, 0, 182], [-64, 0, 200], [-64, 0, 188]]) + '" fill="' + shade(accent, 0.95) + '"/>';
rotor += '<path d="' + poly([[0, 0, 182], [36, 36, 192], [30, 30, 186]]) + '" fill="' + shade(accent, 0.85) + '"/>';
rotor += '<path d="' + poly([[0, 0, 182], [30, -32, 190], [24, -26, 184]]) + '" fill="' + shade(accent, 0.75) + '"/>';
parts.push('<g><animateTransform attributeName="transform" type="rotate" values="360 ' + hub[0].toFixed(0) + ' ' + hub[1].toFixed(0) + ';0 ' + hub[0].toFixed(0) + ' ' + hub[1].toFixed(0) + '" dur="7s" repeatCount="indefinite"/>' + rotor + '</g>');
parts.push(box(-8, -6, 14, 16, 14, 18, '#d4d4d8'));
for (let i = 0; i < 3; i++) {
  parts.push('<ellipse cx="' + (-90 + i * 80) + '" cy="' + (-170 + i * 30) + '" rx="30" ry="10" fill="#27272a" opacity="0.55"><animate attributeName="cx" values="' + (-90 + i * 80) + ';' + (-90 + i * 80 + 40) + ';' + (-90 + i * 80) + '" dur="' + (9 + i * 2) + 's" repeatCount="indefinite"/></ellipse>');
}

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0d1116"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
