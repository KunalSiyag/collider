export interface IsoPlanterBoxOptions {
  accent?: string;
}
...
export function createIsoPlanterBox(options: IsoPlanterBoxOptions = {}): string {
  const { accent = '#4ade80' } = options;
  const rand = rng(1198482669);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="95" ry="24" fill="#000" opacity="0.35"/>');
parts.push(box(-56, -22, 0, 112, 44, 26, '#7c4a21'));
for (let i = 0; i < 5; i++) parts.push('<path d="' + poly([[-56, 22, 4 + i * 5], [56, 22, 4 + i * 5]]) + '" stroke="#5d3a1a" stroke-width="1.5" fill="none"/>');
parts.push(box(-58, -24, 26, 116, 48, 5, '#8d5a2b'));
parts.push(box(-52, -18, 31, 104, 36, 4, '#3e2723'));
for (let i = 0; i < 6; i++) {
  const px = -44 + i * 18;
  parts.push(cyl(px, -2, 35, 4, 7 + rand() * 5, '#166534', 6));
  parts.push(orb(px, -2, 47, 5.5, shade(accent, 0.85 + rand() * 0.35)));
}
for (let i = 0; i < 3; i++) parts.push(box(-40 + i * 36, 14, 35, 8, 5, 6, '#fefce8'));
parts.push('<rect x="-4" y="-84" width="2.5" height="10" fill="#7dd3fc"><animate attributeName="y" values="-84;-46;-84" dur="1.6s" repeatCount="indefinite"/><animate attributeName="opacity" values="0;1;0" dur="1.6s" repeatCount="indefinite"/></rect>');
parts.push(cyl(70, 20, 26, 8, 16, '#38bdf8', 8));
parts.push('<path d="M70 44 L70 52" stroke="#52525b" stroke-width="3" fill="none"/>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#101408"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
