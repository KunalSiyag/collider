export interface IsoFlowerBedOptions {
  accent?: string;
}
...
export function createIsoFlowerBed(options: IsoFlowerBedOptions = {}): string {
  const { accent = '#f472b6' } = options;
  const rand = rng(1351809316);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="105" ry="26" fill="#14532d" opacity="0.5"/>');
parts.push(box(-56, -30, 0, 112, 60, 22, '#7c4a21'));
parts.push(box(-52, -26, 22, 104, 52, 5, '#3e2723'));
const petals = [accent, '#fbbf24', '#a78bfa', '#fb7185'];
for (let i = 0; i < 9; i++) {
  const fx = -44 + (i % 5) * 22;
  const fy = -18 + Math.floor(i / 5) * 26;
  const fh = 18 + rand() * 14;
  parts.push(box(fx, fy, 27, 3, 3, fh, '#15803d'));
  parts.push(orb(fx + 1.5, fy + 1.5, 27 + fh, 6, petals[i % 4]));
  parts.push(orb(fx + 1.5, fy + 1.5, 27 + fh, 2, '#fde68a'));
}
for (let i = 0; i < 4; i++) parts.push(box(48, -24 + i * 13, 22, 6, 6, 4, '#4ade80'));
parts.push('<g><animate attributeName="opacity" values="1;0.3;1" dur="0.4s" repeatCount="indefinite"/><ellipse cx="40" cy="-70" rx="5" ry="3" fill="#fbbf24" transform="rotate(-20 40 -70)"/><ellipse cx="48" cy="-70" rx="5" ry="3" fill="#fbbf24" transform="rotate(20 48 -70)"/></g>');
parts.push('<g><animateTransform attributeName="transform" type="translate" values="0 0;-14 -10;0 0" dur="5s" repeatCount="indefinite"/><ellipse cx="70" cy="-40" rx="4" ry="2.5" fill="#a78bfa"/><ellipse cx="77" cy="-40" rx="4" ry="2.5" fill="#a78bfa"/></g>');
parts.push(cyl(84, -6, 22, 7, 12, '#38bdf8', 8));
