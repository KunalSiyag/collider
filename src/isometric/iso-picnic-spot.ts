export interface IsoPicnicSpotOptions {
  accent?: string;
}
...
export function createIsoPicnicSpot(options: IsoPicnicSpotOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(3780247294);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="115" ry="30" fill="#166534" opacity="0.45"/>');
parts.push(box(-64, -44, 0, 128, 88, 3, '#fafaf9'));
for (let i = 0; i < 8; i++) parts.push('<path d="' + poly([[-64 + i * 16, -44, 3], [-64 + i * 16, 44, 3]]) + '" stroke="' + accent + '" stroke-width="7" opacity="0.55" fill="none"/>');
for (let j = 0; j < 5; j++) parts.push('<path d="' + poly([[-64, -44 + j * 18, 3.2], [64, -44 + j * 18, 3.2]]) + '" stroke="#fca5a5" stroke-width="2" opacity="0.4" fill="none"/>');
let basket = box(-46, -26, 3, 40, 30, 22, '#a16207');
basket += box(-48, -28, 25, 44, 34, 5, '#8d5a2b');
basket += '<path d="M' + project(-30, -8, 30)[0].toFixed(1) + ' ' + project(-30, -8, 30)[1].toFixed(1) + ' Q' + project(-26, -11, 44)[0].toFixed(1) + ' ' + project(-26, -11, 44)[1].toFixed(1) + ' ' + project(-22, -8, 30)[0].toFixed(1) + ' ' + project(-22, -8, 30)[1].toFixed(1) + '" stroke="#713f12" stroke-width="3" fill="none"/>';
parts.push(basket);
parts.push(box(6, 8, 3, 40, 9, 8, '#d19a4a'));
parts.push(box(10, 10, 11, 3, 5, 2, '#8a5a22'));
for (let i = 0; i < 7; i++) parts.push(orb(34 + (i % 3) * 7, -18 + Math.floor(i / 3) * 7, 8, 4, '#7e22ce'));
parts.push(cyl(58, 18, 3, 9, 18, '#bae6fd', 8));
parts.push('<ellipse cx="' + project(58, 18, 21)[0].toFixed(1) + '" cy="' + project(58, 18, 21)[1].toFixed(1) + '" rx="8" ry="4" fill="#fde047"/>');
parts.push('<path d="M' + project(58, 18, 23)[0].toFixed(1) + ' ' + project(58, 18, 23)[1].toFixed(1) + ' l10 -16" stroke="#ef4444" stroke-width="2.5" stroke-linecap="round" fill="none"/>');
parts.push('<circle cx="-70" cy="30" r="1.6" fill="#1c1917"><animate attributeName="cx" values="-70;-50;-70" dur="6s" repeatCount="indefinite"/></circle>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#101510"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
