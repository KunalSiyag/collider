export interface IsoCandyJarOptions {
  accent?: string;
}
...
export function createIsoCandyJar(options: IsoCandyJarOptions = {}): string {
  const { accent = '#f472b6' } = options;
  const rand = rng(1733354226);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="66" rx="70" ry="18" fill="#000" opacity="0.35"/>');
parts.push(cyl(0, 0, 0, 36, 8, '#e2e8f0', 10));
const cols = [accent, '#22d3ee', '#fbbf24', '#4ade80'];
for (let i = 0; i < 14; i++) {
  const a = rand() * Math.PI * 2;
  const rr = rand() * 22;
  parts.push(orb(Math.cos(a) * rr, Math.sin(a) * rr, 14 + rand() * 40, 5.5, cols[i % 4]));
}
parts.push('<g opacity="0.35">' + orb(0, 0, 40, 33, '#bfdbfe') + '</g>');
parts.push(cyl(0, 0, 72, 21, 9, accent, 10));
parts.push(orb(0, 0, 85, 6, shade(accent, 0.8)));
parts.push(orb(46, 20, 4, 5, '#22d3ee'));
parts.push(orb(54, 12, 4, 4.4, '#fbbf24'));
parts.push(orb(-50, 16, 4, 4.6, '#4ade80'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#170f16"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
