export interface IsoBirthdayCakeOptions {
  accent?: string;
}
...
export function createIsoBirthdayCake(options: IsoBirthdayCakeOptions = {}): string {
  const { accent = '#f472b6' } = options;
  const rand = rng(2260364579);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="86" ry="24" fill="#000" opacity="0.3"/>');
parts.push(cyl(0, 0, 0, 54, 6, '#e5e7eb', 12));
parts.push(box(-36, -28, 6, 72, 56, 26, accent));
parts.push(box(-36, -28, 32, 72, 56, 5, '#fdf2f8'));
for (let i = 0; i < 6; i++) parts.push(orb(-32 + i * 13, -26, 37, 3.4, '#fdf2f8'));
parts.push(box(-23, -18, 37, 46, 36, 21, shade(accent, 0.85)));
parts.push(box(-23, -18, 58, 46, 36, 5, '#fdf2f8'));
for (let i = 0; i < 4; i++) {
  const x = -15 + i * 10;
  parts.push(box(x, -6, 63, 3, 3, 14, ['#fbbf24', '#4ade80', '#22d3ee', '#f87171'][i]));
  const fp = project(x + 1.5, -4.5, 79);
  parts.push('<circle cx="' + fp[0].toFixed(1) + '" cy="' + fp[1].toFixed(1) + '" r="3.4" fill="#fbbf24"><animate attributeName="r" values="3.4;5;3.4" dur="' + (0.7 + i * 0.13) + 's" repeatCount="indefinite"/></circle>');
}
for (let i = 0; i < 12; i++) {
  parts.push(orb(rand() * 60 - 30, rand() * 40 - 20, 63.5, 1.4, ['#4ade80', '#22d3ee', '#fbbf24'][i % 3]));
}

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#1c1220"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
