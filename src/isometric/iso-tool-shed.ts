export interface IsoToolShedOptions {
  accent?: string;
}
...
export function createIsoToolShed(options: IsoToolShedOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(3756894424);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="80" rx="110" ry="28" fill="#166534" opacity="0.35"/>');
parts.push(box(-52, -30, 0, 104, 60, 62, '#92400e'));
for (let i = 0; i < 5; i++) parts.push('<path d="' + poly([[-52, 30, 10 + i * 11], [52, 30, 10 + i * 11]]) + '" stroke="#78350f" stroke-width="2" fill="none"/>');
parts.push(pyr(-58, -38, 62, 116, 76, 26, '#7f1d1d'));
parts.push('<path d="' + poly([[-38, 30.5, 10], [-4, 30.5, 10], [-4, 30.5, 54], [-38, 30.5, 54]]) + '" fill="#78350f"/>');
parts.push('<path d="M-38 10 L-4 54 M-4 10 L-38 54" stroke="#5d2510" stroke-width="3" fill="none"/>');
parts.push(orb(-21, 30, 34, 2.5, '#d6d3d1'));
parts.push('<path d="' + poly([[10, 30.4, 26], [30, 30.4, 26], [30, 30.4, 46], [10, 30.4, 46]]) + '" fill="' + accent + '"><animate attributeName="opacity" values="1;0.6;1" dur="2.8s" repeatCount="indefinite"/></path>');
parts.push('<path d="M64 60 L64 -10 M58 60 L60 -4" stroke="#78716c" stroke-width="4" stroke-linecap="round" fill="none"/>');
parts.push(box(56, -12, -8, 16, 8, 6, '#a1a1aa'));
parts.push('<path d="M74 60 L74 6" stroke="#8d5a2b" stroke-width="4" stroke-linecap="round" fill="none"/><path d="M67 8 h14 v-6 h-14 Z" fill="#52525b"/>');
for (let i = 0; i < 4; i++) {
  parts.push(cyl(-68 + i * 11, 42, 0, 5.5, 5, i % 2 ? '#a16207' : '#8d5a2b', 8));
}

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#101210"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
