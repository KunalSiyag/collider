export interface IsoArmchairOptions {
  accent?: string;
}
...
export function createIsoArmchair(options: IsoArmchairOptions = {}): string {
  const { accent = '#f472b6' } = options;
  const rand = rng(2146216886);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="10" rx="84" ry="24" fill="#000" opacity="0.35"/>');
parts.push(box(-44, -32, 0, 88, 66, 18, '#5d4037'));
parts.push(box(-44, -32, 18, 10, 66, 36, '#4e342e'));
parts.push(box(34, -32, 18, 10, 66, 36, '#4e342e'));
parts.push(box(-38, -34, 18, 76, 12, 48, shade(accent, 0.75)));
parts.push(box(-38, -26, 18, 76, 54, 14, accent));
parts.push(box(-34, -20, 32, 68, 46, 4, shade(accent, 1.18)));
parts.push(box(-30, -30, 34, 60, 7, 6, shade(accent, 0.65)));
parts.push(orb(-16, -16, 42, 9, shade(accent, 1.35)));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#171225"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
