export interface IsoCargoCraneOptions {
  accent?: string;
}
...
export function createIsoCargoCrane(options: IsoCargoCraneOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(1126493743);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="90" rx="130" ry="30" fill="#000" opacity="0.35"/>');
parts.push(box(-14, -12, 0, 28, 24, 122, '#f59e0b'));
parts.push('<path d="M-14 12 L14 34 M-14 34 L14 56 M-14 56 L14 78 M-14 78 L14 100" stroke="#b45309" stroke-width="2.5" fill="none"/>');
parts.push(box(-20, -16, 122, 48, 30, 22, '#27272a'));
parts.push('<path d="' + poly([[-16, 13, 126], [22, 13, 126], [22, 13, 138], [-16, 13, 138]]) + '" fill="#bae6fd"/>');
parts.push(box(-20, -8, 144, 128, 14, 10, '#f59e0b'));
parts.push(box(-78, -8, 144, 58, 14, 10, '#b45309'));
parts.push(box(-70, -6, 154, 30, 10, 16, '#52525b'));
parts.push('<path d="M4 -150 L-64 -148 M4 -150 L102 -148" stroke="#fbbf24" stroke-width="2" fill="none"/>');
const pivot = project(88, -1, 152);
let hang = box(70, -16, 44, 38, 24, 24, accent);
hang += '<path d="M' + pivot[0].toFixed(1) + ' ' + pivot[1].toFixed(1) + ' L' + project(89, -4, 68)[0].toFixed(1) + ' ' + project(89, -4, 68)[1].toFixed(1) + '" stroke="#d4d4d8" stroke-width="2" fill="none"/>';
parts.push('<g><animateTransform attributeName="transform" type="rotate" values="-3 ' + pivot[0].toFixed(0) + ' ' + pivot[1].toFixed(0) + ';3 ' + pivot[0].toFixed(0) + ' ' + pivot[1].toFixed(0) + ';-3 ' + pivot[0].toFixed(0) + ' ' + pivot[1].toFixed(0) + '" dur="6s" repeatCount="indefinite"/>' + hang + '</g>');
parts.push(box(40, 30, 0, 38, 24, 22, '#7f1d1d'));
parts.push(box(44, -6, 0, 38, 24, 22, '#4d7c0f'));
parts.push(led(-4, -2, 168, '#ef4444', 1.4, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#101319"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
