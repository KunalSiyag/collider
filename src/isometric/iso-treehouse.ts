export interface IsoTreehouseOptions {
  accent?: string;
}
...
export function createIsoTreehouse(options: IsoTreehouseOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(520533975);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="96" rx="110" ry="28" fill="#14532d" opacity="0.45"/>');
function trunk(x, y, w, h) {
  return box(x, y, 0, w, w, h, '#5d4037');
}
parts.push(trunk(-10, -8, 22, 120));
parts.push(orb(-30, -30, 130, 22, '#166534'));
parts.push(orb(24, 16, 138, 19, '#15803d'));
parts.push(orb(0, 0, 152, 21, '#22c55e'));
parts.push(box(-44, -30, 84, 88, 60, 8, '#8d5a2b'));
for (const [lx, ly] of [[-40, -26], [-40, 22], [36, -26], [36, 22]]) parts.push(box(lx, ly, 0, 8, 8, 84, '#78350f'));
parts.push(box(-38, -26, 92, 76, 52, 40, '#a16207'));
parts.push('<path d="' + poly([[-38, 26, 98], [-6, 26, 98], [-6, 26, 126], [-38, 26, 126]]) + '" fill="#fde68a"><animate attributeName="opacity" values="1;0.7;1" dur="2.6s" repeatCount="indefinite"/></path>');
parts.push(pyr(-44, -34, 132, 88, 60, 18, '#7f1d1d'));
for (let i = 0; i < 6; i++) parts.push('<path d="M' + (-20 + i * 7) + ' 84 L' + (-20 + i * 7) + ' ' + (84 + i * 1) + '" stroke="#a16207" stroke-width="3" fill="none" transform="translate(0 ' + (-i * 13) + ')"/>');
parts.push('<path d="M-22 84 L-22 8 M-6 86 L-6 12 M-24 70 L-4 72 M-25 54 L-5 56 M-26 38 L-6 40" stroke="#d6b68c" stroke-width="2.5" fill="none"/>');
parts.push('<path d="M44 96 Q80 90 96 60" stroke="#d6d3d1" stroke-width="2" fill="none"/>');
let bucket = box(88, 52, 44, 12, 12, 10, '#78716c') + orb(94, 58, 55, 3, '#a1a1aa');
parts.push('<g><animateTransform attributeName="transform" type="translate" values="0 0;0 -18;0 -18;0 0" keyTimes="0;0.4;0.6;1" dur="6s" repeatCount="indefinite"/>' + bucket + '</g>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0e1310"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
