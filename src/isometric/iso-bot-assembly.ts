export interface IsoBotAssemblyOptions {
  accent?: string;
}
...
export function createIsoBotAssembly(options: IsoBotAssemblyOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(3742880341);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="115" ry="28" fill="#000" opacity="0.35"/>');
parts.push(box(-72, -18, 18, 144, 36, 8, '#3f3f46'));
for (let i = 0; i < 5; i++) parts.push(led(-58 + i * 28, -18, 27, '#52525b'));
parts.push(box(-64, 12, 0, 10, 10, 18, '#27272a'));
parts.push(box(54, 12, 0, 10, 10, 18, '#27272a'));
parts.push(box(-30, -12, 26, 24, 22, 20, '#a1a1aa'));
parts.push(led(-22, -6, 40, accent, 1.4, 0));
parts.push(box(8, -10, 26, 24, 22, 20, '#a1a1aa'));
parts.push(led(16, -4, 40, '#f472b6', 1.4, 0.5));
parts.push(cyl(66, 22, 0, 11, 28, '#52525b', 8));
parts.push(box(58, 14, 28, 14, 14, 32, '#71717a'));
parts.push(box(30, 18, 52, 30, 10, 9, '#a1a1aa'));
parts.push(box(24, 14, 61, 8, 6, 10, '#52525b'));
const wp = project(28, 16, 66);
parts.push('<circle cx="' + wp[0].toFixed(1) + '" cy="' + wp[1].toFixed(1) + '" r="4" fill="' + accent + '"><animate attributeName="opacity" values="1;0.2;1" dur="0.6s" repeatCount="indefinite"/></circle>');
parts.push('<circle cx="36" cy="-96" r="2" fill="#fbbf24"><animate attributeName="cy" values="-96;-130;-96" dur="1.2s" repeatCount="indefinite"/><animate attributeName="opacity" values="1;0;1" dur="1.2s" repeatCount="indefinite"/></circle>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#10141c"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
