export interface IsoCastleKeepOptions {
  accent?: string;
}
...
export function createIsoCastleKeep(options: IsoCastleKeepOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(3377465913);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="80" rx="125" ry="30" fill="#14532d" opacity="0.6"/>');
parts.push(box(-70, -42, 0, 140, 84, 14, '#365314'));
parts.push(box(-42, -32, 14, 84, 64, 72, '#78716c'));
for (let i = 0; i < 6; i++) parts.push(box(-42 + i * 14.5, -32, 86, 9, 8, 10, '#8a8a85'));
for (let i = 0; i < 6; i++) parts.push(box(-42 + i * 14.5, 24, 86, 9, 8, 10, '#6b6a65'));
parts.push('<path d="' + poly([[-14, 32, 20], [14, 32, 20], [14, 32, 52], [-14, 32, 52]]) + '" fill="#1c1917"/>');
parts.push('<path d="M-14 32 Q0 14 14 32" stroke="#1c1917" stroke-width="8" fill="none"/>');
for (let i = 0; i < 3; i++) {
  parts.push('<path d="' + poly([[-30 + i * 28, 32.3, 60], [-22 + i * 28, 32.3, 60], [-22 + i * 28, 32.3, 74], [-30 + i * 28, 32.3, 74]]) + '" fill="#fbbf24"><animate attributeName="opacity" values="1;0.5;1" dur="' + (2 + i) + 's" repeatCount="indefinite"/></path>');
}
parts.push(cyl(52, -30, 14, 17, 88, '#a8a29e', 8));
parts.push(pyr(35, -47, 102, 34, 34, 26, '#9f1239'));
parts.push('<path d="M52 -30 L52 -146" stroke="#78350f" stroke-width="3" fill="none"/>');
parts.push('<polygon points="' + [project(53, -30, 144), project(53, -30, 132), project(83, -30, 138)].map((p) => p[0].toFixed(1) + ',' + p[1].toFixed(1)).join(' ') + '" fill="' + accent + '"><animate attributeName="opacity" values="1;0.7;1" dur="1.8s" repeatCount="indefinite"/></polygon>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#111320"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
