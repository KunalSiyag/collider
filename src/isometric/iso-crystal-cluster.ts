export interface IsoCrystalClusterOptions {
  accent?: string;
}
...
export function createIsoCrystalCluster(options: IsoCrystalClusterOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(854292986);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="90" ry="26" fill="#000" opacity="0.35"/>');
parts.push(pyr(-52, -30, 0, 104, 62, 22, '#3f3f46'));
function spike(x, y, w, h, col) {
  return pyr(x, y, 16, w, w, h, col);
}
parts.push(spike(-30, -18, 26, 84, accent));
parts.push(spike(-4, -6, 20, 116, '#8b5cf6'));
parts.push(spike(18, 2, 24, 64, '#f472b6'));
parts.push(spike(-44, 4, 16, 42, shade(accent, 0.85)));
parts.push(spike(38, -8, 13, 34, '#a78bfa'));
for (let i = 0; i < 8; i++) {
  const sx = rand() * 140 - 70;
  parts.push('<circle cx="' + sx.toFixed(0) + '" cy="' + (-40 - rand() * 100).toFixed(0) + '" r="2" fill="' + ['#22d3ee', '#c4b5fd', '#f9a8d4'][i % 3] + '"><animate attributeName="opacity" values="0;1;0" dur="' + (1.6 + rand() * 2).toFixed(1) + 's" begin="' + (rand() * 2).toFixed(1) + 's" repeatCount="indefinite"/></circle>');
}
parts.push('<ellipse cx="0" cy="40" rx="80" ry="20" fill="' + accent + '" opacity="0.06"><animate attributeName="ry" values="20;24;20" dur="4s" repeatCount="indefinite"/></ellipse>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#101024"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
