export interface IsoMilkCratesOptions {
  accent?: string;
}
...
export function createIsoMilkCrates(options: IsoMilkCratesOptions = {}): string {
  const { accent = '#e2e8f0' } = options;
  const rand = rng(3788011253);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="76" rx="85" ry="22" fill="#000" opacity="0.35"/>');
function crate(x, y, z, col) {
  let s = box(x, y, z, 44, 36, 26, col);
  s += '<path d="' + poly([[x + 4, y + 31, z + 27], [x + 40, y + 31, z + 27]]) + '" stroke="' + shade(col, 0.6) + '" stroke-width="2" fill="none"/>';
  return s;
}
parts.push(crate(-30, -20, 0, '#3b82f6'));
parts.push(crate(18, -8, 0, '#3b82f6'));
parts.push(crate(-6, -14, 26, '#2563eb'));
function bottle(x, y, z) {
  let s = cyl(x, y, z, 5, 14, '#f8fafc', 8);
  s += orb(x, y, z + 17, 3, '#fca5a5');
  return s;
}
parts.push(bottle(-20, -8, 52));
parts.push(bottle(-4, 2, 52));
parts.push(bottle(10, -8, 52));
parts.push(bottle(40, 16, 0));
parts.push(bottle(52, 8, 0));
parts.push('<ellipse cx="-58" cy="40" rx="20" ry="8" fill="#f8fafc" opacity="0.25"/>');
parts.push('<ellipse cx="-52" cy="38" rx="8" ry="3" fill="#f8fafc" opacity="0.35"/>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#101218"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
