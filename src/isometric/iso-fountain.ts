export interface IsoFountainOptions {
  accent?: string;
}
...
export function createIsoFountain(options: IsoFountainOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(3901221001);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="40" rx="120" ry="32" fill="#3f3f46" opacity="0.6"/>');
parts.push(cyl(0, 0, 0, 62, 14, '#78716c', 12));
parts.push('<ellipse cx="0" cy="-10" rx="56" ry="28" fill="#0ea5e9"><animate attributeName="ry" values="28;26;28" dur="3s" repeatCount="indefinite"/></ellipse>');
for (let i = 1; i <= 2; i++) {
  parts.push('<ellipse cx="0" cy="-10" rx="8" ry="4" fill="none" stroke="#7dd3fc" stroke-width="1.5"><animate attributeName="rx" values="6;' + 20 * i + '" dur="' + (2 + i) + 's" repeatCount="indefinite"/><animate attributeName="ry" values="3;' + 10 * i + '" dur="' + (2 + i) + 's" repeatCount="indefinite"/><animate attributeName="opacity" values="0.8;0" dur="' + (2 + i) + 's" repeatCount="indefinite"/></ellipse>');
}
parts.push(cyl(0, 0, 14, 10, 34, '#a8a29e', 10));
parts.push(cyl(0, 0, 48, 24, 6, '#a8a29e', 12));
parts.push(cyl(0, 0, 54, 8, 22, '#78716c', 10));
parts.push(cyl(0, 0, 76, 15, 5, '#a8a29e', 10));
parts.push(orb(0, 0, 86, 7, accent));
const jp = project(0, 0, 92);
parts.push('<path d="M' + jp[0].toFixed(1) + ' ' + jp[1].toFixed(1) + ' q-8 -22 0 -34 M' + jp[0].toFixed(1) + ' ' + jp[1].toFixed(1) + ' q10 -18 4 -30" stroke="#7dd3fc" stroke-width="2" fill="none"><animate attributeName="opacity" values="0.8;0.3;0.8" dur="1.6s" repeatCount="indefinite"/></path>');
for (let i = 0; i < 5; i++) parts.push(orb(-44 + i * 22, 52, 17, 2.4, i % 2 ? '#fde68a' : '#94a3b8'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0e141c"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
