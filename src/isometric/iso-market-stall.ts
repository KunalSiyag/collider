export interface IsoMarketStallOptions {
  accent?: string;
}
...
export function createIsoMarketStall(options: IsoMarketStallOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(1158610690);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="115" ry="28" fill="#000" opacity="0.35"/>');
parts.push(box(-58, -26, 0, 116, 52, 44, '#7c4a21'));
parts.push(box(-62, -30, 44, 124, 60, 6, '#8d5a2b'));
parts.push(box(-58, -34, 50, 10, 10, 66, '#5d4037'));
parts.push(box(48, -34, 50, 10, 10, 66, '#5d4037'));
for (let i = 0; i < 6; i++) parts.push(box(-62 + i * 21, -34, 112, 21, 56, 5, i % 2 ? '#fef3c7' : accent));
const fruits = ['#ef4444', '#fbbf24', '#84cc16'];
for (let cIdx = 0; cIdx < 3; cIdx++) {
  const cx = -40 + cIdx * 40;
  parts.push(box(cx, -22, 50, 32, 26, 14, '#b45309'));
  for (let i = 0; i < 6; i++) {
    parts.push(orb(cx + 5 + (i % 3) * 11, -18 + Math.floor(i / 3) * 12, 64 + rand() * 4, 5.5, fruits[cIdx]));
  }
}
parts.push('<path d="M74 20 L74 -60 M74 -46 L92 -46" stroke="#78716c" stroke-width="3" fill="none"/>');
parts.push(cyl(90, 16, 20, 9, 2, '#d6d3d1', 10));
parts.push(led(0, -29, 118, '#fbbf24', 2.4, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#141009"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
