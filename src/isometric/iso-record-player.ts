export interface IsoRecordPlayerOptions {
  accent?: string;
}
...
export function createIsoRecordPlayer(options: IsoRecordPlayerOptions = {}): string {
  const { accent = '#f472b6' } = options;
  const rand = rng(3796293022);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="66" rx="95" ry="24" fill="#000" opacity="0.35"/>');
parts.push(box(-58, -34, 0, 116, 68, 14, '#6d4c41'));
parts.push(cyl(-12, 0, 14, 40, 4, '#18181b', 16));
parts.push('<ellipse cx="' + project(-12, 0, 19)[0].toFixed(1) + '" cy="' + project(-12, 0, 19)[1].toFixed(1) + '" rx="38" ry="19" fill="#27272a"/>');
for (let g = 1; g <= 3; g++) parts.push('<ellipse cx="' + project(-12, 0, 19)[0].toFixed(1) + '" cy="' + project(-12, 0, 19)[1].toFixed(1) + '" rx="' + (36 - g * 7) + '" ry="' + (18 - g * 3.5) + '" fill="none" stroke="#3f3f46" stroke-width="0.8"/>');
parts.push('<ellipse cx="' + project(-12, 0, 19)[0].toFixed(1) + '" cy="' + project(-12, 0, 19)[1].toFixed(1) + '" rx="10" ry="5" fill="' + accent + '"/>');
parts.push('<circle cx="' + (project(-12, 0, 19)[0] + 6).toFixed(1) + '" cy="' + (project(-12, 0, 19)[1] - 2).toFixed(1) + '" r="1.5" fill="#fafaf9"/>');
parts.push(orb(38, -20, 18, 5, '#a8a29e'));
parts.push('<path d="M' + project(38, -20, 18)[0].toFixed(1) + ' ' + project(38, -20, 18)[1].toFixed(1) + ' L' + project(20, -6, 24)[0].toFixed(1) + ' ' + project(20, -6, 24)[1].toFixed(1) + '" stroke="#e7e5e4" stroke-width="3" fill="none"/>');
parts.push(box(16, -10, 21, 8, 8, 4, '#d6d3d1'));
parts.push(led(50, 24, 17, '#4ade80', 2.4, 0));
for (let i = 0; i < 3; i++) {
  parts.push('<circle cx="' + (76 - i * 8) + '" cy="' + (-60 - i * 16) + '" r="2.4" fill="#f9a8d4"><animate attributeName="cy" values="' + (-60 - i * 16) + ';' + (-110 - i * 8) + ';' + (-60 - i * 16) + '" dur="' + (2.4 + i * 0.6) + 's" repeatCount="indefinite"/><animate attributeName="opacity" values="0;1;0" dur="' + (2.4 + i * 0.6) + 's" repeatCount="indefinite"/></circle>');
}
