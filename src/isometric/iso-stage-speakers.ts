export interface IsoStageSpeakersOptions {
  accent?: string;
}
...
export function createIsoStageSpeakers(options: IsoStageSpeakersOptions = {}): string {
  const { accent = '#8b5cf6' } = options;
  const rand = rng(1213010930);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="90" rx="130" ry="30" fill="#000" opacity="0.4"/>');
function stack(sx) {
  let s = box(sx, -18, 0, 40, 36, 36, '#18181b');
  s += box(sx, -18, 36, 40, 36, 40, '#27272a');
  const d1 = project(sx + 20, 18, 54);
  s += '<ellipse cx="' + d1[0].toFixed(1) + '" cy="' + d1[1].toFixed(1) + '" rx="14" ry="7" fill="#3f3f46"/><ellipse cx="' + d1[0].toFixed(1) + '" cy="' + d1[1].toFixed(1) + '" rx="6" ry="3" fill="#a1a1aa"/>';
  return s;
}
parts.push(stack(-78));
parts.push(stack(38));
parts.push(box(-19, -14, 0, 38, 28, 10, '#3f3f46'));
for (let i = 0; i < 5; i++) parts.push('<rect x="' + (-14 + i * 7) + '" y="-104" width="3" height="' + (6 + rand() * 10).toFixed(0) + '" fill="#4ade80"><animate attributeName="height" values="' + (6 + rand() * 10).toFixed(0) + ';2;' + (6 + rand() * 10).toFixed(0) + '" dur="0.6s" repeatCount="indefinite"/></rect>');
parts.push('<path d="M0 -150 L0 -60 M-14 -60 L14 -60 M0 -64 L0 -20" stroke="#71717a" stroke-width="4" fill="none"/>');
parts.push(cyl(0, 0, -22, 8, 8, '#3f3f46', 8));
parts.push('<g transform="rotate(-24 0 -140)"><polygon points="-6,-140 -150,-190 -150,-84" fill="#8b5cf6" opacity="0.1"><animate attributeName="opacity" values="0.1;0.02;0.1" dur="3s" repeatCount="indefinite"/></polygon></g>');
parts.push('<g transform="rotate(24 0 -140)"><polygon points="6,-140 150,-190 150,-84" fill="#22d3ee" opacity="0.1"><animate attributeName="opacity" values="0.02;0.1;0.02" dur="3s" repeatCount="indefinite"/></polygon></g>');
parts.push(puff(-50, 10, 20, 14, 4, 0, '#64748b'));
parts.push(puff(50, 10, 20, 16, 5, 1.6, '#64748b'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0d0b16"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
