export interface IsoFireHydrantOptions {
  accent?: string;
}
...
export function createIsoFireHydrant(options: IsoFireHydrantOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(870031094);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="80" ry="20" fill="#000" opacity="0.35"/>');
parts.push(box(-70, -40, 0, 140, 80, 6, '#3f3f46'));
parts.push(cyl(0, 0, 6, 22, 8, shade(accent, 0.8), 10));
parts.push(cyl(0, 0, 14, 16, 52, accent, 10));
parts.push(orb(0, 0, 70, 17, shade(accent, 1.1)));
parts.push(orb(0, 0, 80, 7, shade(accent, 0.75)));
parts.push(cyl(0, 0, 78, 5, 6, shade(accent, 0.9), 8));
const lc = project(-18, 0, 40);
const rc = project(18, 0, 40);
parts.push('<circle cx="' + lc[0].toFixed(1) + '" cy="' + lc[1].toFixed(1) + '" r="7" fill="' + shade(accent, 0.85) + '"/>');
parts.push('<circle cx="' + rc[0].toFixed(1) + '" cy="' + rc[1].toFixed(1) + '" r="7" fill="' + shade(accent, 0.85) + '"/>');
parts.push('<path d="M' + (lc[0] + 7).toFixed(1) + ' ' + lc[1].toFixed(1) + ' Q' + lc[0].toFixed(1) + ' ' + (lc[1] + 16).toFixed(1) + ' ' + rc[0].toFixed(1) + ' ' + (rc[1] + 14).toFixed(1) + '" stroke="#d6d3d1" stroke-width="2" fill="none"/>');
for (let i = 0; i < 5; i++) parts.push(orb(-60 + i * 14, 34, 8, 2.4, '#4d7c0f'));
