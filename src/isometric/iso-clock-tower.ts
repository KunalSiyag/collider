export interface IsoClockTowerOptions {
  accent?: string;
}
...
export function createIsoClockTower(options: IsoClockTowerOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(3599059777);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="106" rx="70" ry="18" fill="#000" opacity="0.35"/>');
parts.push(box(-24, -18, 0, 48, 36, 118, '#57534e'));
parts.push(box(-26, -20, 118, 52, 40, 8, '#44403c'));
const fL = project(0, 18, 88);
const fR = project(24, 0, 88);
parts.push('<circle cx="' + fL[0].toFixed(1) + '" cy="' + fL[1].toFixed(1) + '" r="14" fill="#fafaf9"/><path d="M' + fL[0].toFixed(1) + ' ' + fL[1].toFixed(1) + ' L' + (fL[0] + 8).toFixed(1) + ' ' + (fL[1] - 5).toFixed(1) + ' M' + fL[0].toFixed(1) + ' ' + fL[1].toFixed(1) + ' L' + (fL[0] - 4).toFixed(1) + ' ' + (fL[1] + 9).toFixed(1) + '" stroke="#1c1917" stroke-width="2"/>');
parts.push('<circle cx="' + fR[0].toFixed(1) + '" cy="' + fR[1].toFixed(1) + '" r="14" fill="#fafaf9"/><path d="M' + fR[0].toFixed(1) + ' ' + fR[1].toFixed(1) + ' L' + (fR[0] + 8).toFixed(1) + ' ' + (fR[1] - 5).toFixed(1) + ' M' + fR[0].toFixed(1) + ' ' + fR[1].toFixed(1) + ' L' + (fR[0] - 4).toFixed(1) + ' ' + (fR[1] + 9).toFixed(1) + '" stroke="#1c1917" stroke-width="2"/>');
parts.push('<path d="' + poly([[-10, 19, 128], [10, 19, 128], [10, 19, 142], [-10, 19, 142]]) + '" fill="#1c1917"/>');
const bellP = project(0, 14, 136);
parts.push('<g><animateTransform attributeName="transform" type="rotate" values="-10 ' + bellP[0].toFixed(0) + ' ' + (bellP[1] - 10).toFixed(0) + ';10 ' + bellP[0].toFixed(0) + ' ' + (bellP[1] - 10).toFixed(0) + ';-10 ' + bellP[0].toFixed(0) + ' ' + (bellP[1] - 10).toFixed(0) + '" dur="2.4s" repeatCount="indefinite"/><circle cx="' + bellP[0].toFixed(1) + '" cy="' + bellP[1].toFixed(1) + '" r="6" fill="' + accent + '"/></g>');
parts.push(pyr(-28, -22, 126, 56, 44, 30, '#7f1d1d'));
parts.push(orb(0, 0, 160, 5, accent));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0f1018"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
