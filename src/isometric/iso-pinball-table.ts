export interface IsoPinballTableOptions {
  accent?: string;
}
...
export function createIsoPinballTable(options: IsoPinballTableOptions = {}): string {
  const { accent = '#8b5cf6' } = options;
  const rand = rng(2614865844);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="80" rx="105" ry="26" fill="#000" opacity="0.35"/>');
let table = box(-52, -30, 0, 104, 60, 14, '#312e81');
table += '<path d="' + poly([[-48, -26, 14], [48, -26, 14], [48, 26, 14], [-48, 26, 14]]) + '" fill="#1e1b4b"/>';
for (let i = 0; i < 4; i++) table += '<path d="' + poly([[-44, -24 + i * 14, 15], [40, -24 + i * 14, 15]]) + '" stroke="' + accent + '" stroke-width="1.5" opacity="0.5" fill="none"/>';
const bpos = [[-20, -10], [10, -16], [22, 2]];
for (const [bx, by] of bpos) {
  const bp = project(bx, by, 15);
  table += '<circle cx="' + bp[0].toFixed(1) + '" cy="' + bp[1].toFixed(1) + '" r="7" fill="#ec4899"><animate attributeName="opacity" values="1;0.4;1" dur="1.2s" repeatCount="indefinite"/></circle><circle cx="' + bp[0].toFixed(1) + '" cy="' + bp[1].toFixed(1) + '" r="3" fill="#fbcfe8"/>';
}
table += '<path d="M' + project(-6, 24, 15)[0].toFixed(1) + ' ' + project(-6, 24, 15)[1].toFixed(1) + ' l-16 8" stroke="#e7e5e4" stroke-width="5" stroke-linecap="round"/><path d="M' + project(6, 24, 15)[0].toFixed(1) + ' ' + project(6, 24, 15)[1].toFixed(1) + ' l16 8" stroke="#e7e5e4" stroke-width="5" stroke-linecap="round"/>';
parts.push(table);
parts.push(box(-56, -34, 14, 112, 68, 40, '#18181b'));
for (let i = 0; i < 5; i++) parts.push(led(-36 + i * 18, 0.5, 46, ['#fbbf24', accent, '#22d3ee'][i % 3], 0.9, i * 0.15));
parts.push('<circle cx="' + project(0, -20, 40)[0].toFixed(1) + '" cy="' + project(0, -20, 40)[1].toFixed(1) + '" r="9" fill="' + accent + '" opacity="0.85"><animate attributeName="r" values="9;11;9" dur="1.6s" repeatCount="indefinite"/></circle>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#120d18"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
