export interface IsoBlenderOptions {
  accent?: string;
}
...
export function createIsoBlender(options: IsoBlenderOptions = {}): string {
  const { accent = '#4ade80' } = options;
  const rand = rng(4106494131);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="76" rx="72" ry="20" fill="#000" opacity="0.35"/>');
parts.push(box(-22, -16, 0, 44, 32, 26, '#27272a'));
parts.push(box(-14, -16, 26, 28, 24, 46, '#bae6fd'));
parts.push('<g opacity="0.85">' + box(-11, -13, 27, 22, 18, 24, accent) + '</g>');
parts.push(box(-15, -17, 72, 30, 26, 7, '#3f3f46'));
parts.push(orb(0, -4, 81, 5, '#52525b'));
for (let i = 0; i < 3; i++) parts.push(led(-8 + i * 8, 15, 10, [accent, '#fbbf24', '#f87171'][i], 1 + i * 0.3, i * 0.2));
parts.push(orb(38, 10, 4, 7, '#ef4444'));
parts.push(orb(46, 2, 4, 6, '#ef4444'));
parts.push(box(34, -12, 2, 16, 6, 5, '#facc15'));
parts.push('<path d="M-30 40 q-14 -8 -6 -20" stroke="#94a3b8" stroke-width="2" fill="none"/>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#11151c"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
