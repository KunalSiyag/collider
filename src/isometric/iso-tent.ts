export interface IsoTentOptions {
  accent?: string;
}
...
export function createIsoTent(options: IsoTentOptions = {}): string {
  const { accent = '#f59e0b' } = options;
  const rand = rng(3628764788);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="66" rx="120" ry="30" fill="#14532d" opacity="0.35"/>');
parts.push('<path d="' + poly([[-52, -20, 0], [0, -20, 0], [0, 6, 56]]) + '" fill="#ca8a04"/>');
parts.push('<path d="' + poly([[52, -20, 0], [0, -20, 0], [0, 6, 56]]) + '" fill="' + shade('#ca8a04', 0.75) + '"/>');
parts.push('<path d="' + poly([[-52, -20, 0], [-52, 24, 0], [0, 6, 56]]) + '" fill="#eab308"/>');
parts.push('<path d="' + poly([[52, 24, 0], [52, -20, 0], [0, 6, 56]]) + '" fill="' + shade('#eab308', 0.85) + '"/>');
parts.push('<path d="' + poly([[-14, 2, 0], [14, 2, 0], [0, 4.5, 34]]) + '" fill="#1c1917"/>');
parts.push('<path d="M-52 -20 L-70 30 M52 -20 L70 30 M0 6 L0 -64 M-52 -20 L0 -64 L52 -20" stroke="#78350f" stroke-width="3" fill="none"/>');
parts.push(box(-6, -2, -60, 12, 4, 4, '#78350f'));
parts.push(cyl(-76, 30, 0, 5, 14, '#52525b', 6));
parts.push(orb(-76, 30, 18, 6, '#fde68a'));
parts.push(box(58, 18, 0, 30, 20, 16, '#713f12'));
parts.push(box(56, 16, 16, 34, 24, 5, '#8d5a2b'));
parts.push('<ellipse cx="' + project(84, 30, 2)[0].toFixed(1) + '" cy="' + project(84, 30, 2)[1].toFixed(1) + '" rx="12" ry="5" fill="#f97316"><animate attributeName="fill" values="#f97316;#fbbf24;#f97316" dur="1.4s" repeatCount="indefinite"/></ellipse>');
parts.push('<circle cx="86" cy="-30" r="2" fill="#fbbf24"><animate attributeName="cy" values="-30;-58;-30" dur="1.8s" repeatCount="indefinite"/><animate attributeName="opacity" values="1;0;1" dur="1.8s" repeatCount="indefinite"/></circle>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0c1014"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
