export interface IsoTeaSetOptions {
  accent?: string;
}
...
export function createIsoTeaSet(options: IsoTeaSetOptions = {}): string {
  const { accent = '#f472b6' } = options;
  const rand = rng(3849694632);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="90" ry="24" fill="#000" opacity="0.35"/>');
parts.push(box(-62, -36, 0, 124, 72, 5, '#7c4a21'));
for (let i = 0; i < 6; i++) parts.push('<path d="' + poly([[-58 + i * 20, -32, 5], [-58 + i * 20, 32, 5]]) + '" stroke="#a16207" stroke-width="2" fill="none"/>');
parts.push(cyl(-16, 4, 5, 22, 22, '#fafaf9', 12));
const spout = project(8, 4, 30);
parts.push('<path d="M' + spout[0].toFixed(1) + ' ' + spout[1].toFixed(1) + ' l14 -6" stroke="#fafaf9" stroke-width="7" stroke-linecap="round"/>');
parts.push('<path d="M' + project(-38, 4, 26)[0].toFixed(1) + ' ' + project(-38, 4, 26)[1].toFixed(1) + ' q-14 10 -2 20" stroke="#fafaf9" stroke-width="5" fill="none"/>');
parts.push(cyl(-16, 4, 27, 8, 7, '#fafaf9', 10));
parts.push(orb(-16, 4, 37, 3.5, accent));
parts.push('<path d="' + poly([[-16, 26, 27], [12, 26, 27]]) + '" stroke="' + accent + '" stroke-width="2.5" fill="none"/>');
parts.push(cyl(28, -18, 5, 9, 12, '#fafaf9', 10));
parts.push('<ellipse cx="' + project(28, -18, 17)[0].toFixed(1) + '" cy="' + project(28, -18, 17)[1].toFixed(1) + '" rx="7" ry="3.5" fill="' + shade(accent, 0.9) + '"/>');
parts.push(cyl(34, 24, 5, 9, 12, '#fafaf9', 10));
parts.push('<ellipse cx="' + project(34, 24, 17)[0].toFixed(1) + '" cy="' + project(34, 24, 17)[1].toFixed(1) + '" rx="7" ry="3.5" fill="' + shade(accent, 0.9) + '"/>');
parts.push(puff(-14, 2, 40, 6, 2.6, 0));
parts.push(puff(-10, 6, 44, 8, 3.2, 1.1));
parts.push(box(52, 8, 5, 9, 9, 8, '#fefce8'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#141018"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
