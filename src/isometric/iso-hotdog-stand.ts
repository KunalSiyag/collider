export interface IsoHotdogStandOptions {
  accent?: string;
}
...
export function createIsoHotdogStand(options: IsoHotdogStandOptions = {}): string {
  const { accent = '#facc15' } = options;
  const rand = rng(839402243);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="66" rx="100" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-44, -22, 0, 88, 44, 38, '#b45309'));
const wc = project(-28, 22, 12);
const wd = project(28, 22, 12);
parts.push('<circle cx="' + wc[0].toFixed(1) + '" cy="' + wc[1].toFixed(1) + '" r="11" fill="#18181b"/><circle cx="' + wc[0].toFixed(1) + '" cy="' + wc[1].toFixed(1) + '" r="4" fill="#78716c"/>');
parts.push('<circle cx="' + wd[0].toFixed(1) + '" cy="' + wd[1].toFixed(1) + '" r="11" fill="#18181b"/><circle cx="' + wd[0].toFixed(1) + '" cy="' + wd[1].toFixed(1) + '" r="4" fill="#78716c"/>');
parts.push(box(-42, -24, 38, 84, 48, 5, '#e7e5e4'));
parts.push(box(-24, -22, 43, 48, 32, 22, '#bae6fd'));
for (let i = 0; i < 3; i++) parts.push(orb(-12 + i * 13, -6, 55, 6, '#d97706'));
for (let i = 0; i < 2; i++) parts.push('<ellipse cx="0" cy="0" rx="0" ry="0"/>');
parts.push(cyl(-34, 8, 43, 5, 10, '#ef4444', 8));
parts.push(cyl(-24, 12, 43, 5, 10, '#4ade80', 8));
parts.push('<path d="M4 43 L4 112" stroke="#57534e" stroke-width="4" fill="none"/>');
parts.push(pyr(-42, -34, 110, 96, 64, 20, accent));
for (let i = 0; i < 6; i++) parts.push(orb(-36 + i * 14, 24, 131, 3, i % 2 ? '#fef9c3' : shade(accent, 0.85)));
parts.push(led(30, -24, 50, '#f97316', 1.8, 0));
