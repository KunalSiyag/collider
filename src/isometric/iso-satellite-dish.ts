export interface IsoSatelliteDishOptions {
  accent?: string;
}
...
export function createIsoSatelliteDish(options: IsoSatelliteDishOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(4007574289);
  const parts: string[] = [];

for (let i = 0; i < 9; i++) parts.push('<circle cx="' + (rand() * 300 - 150).toFixed(0) + '" cy="' + (-rand() * 180 - 60).toFixed(0) + '" r="1.5" fill="#e2e8f0"><animate attributeName="opacity" values="0.2;1;0.2" dur="' + (1.5 + rand() * 2).toFixed(1) + 's" begin="' + (rand() * 2).toFixed(1) + 's" repeatCount="indefinite"/></circle>');
parts.push('<ellipse cx="0" cy="80" rx="90" ry="22" fill="#000" opacity="0.35"/>');
parts.push(box(-26, -20, 0, 52, 40, 14, '#52525b'));
const dc = project(0, 0, 74);
parts.push('<g transform="rotate(-32 ' + dc[0].toFixed(1) + ' ' + dc[1].toFixed(1) + ')">');
parts.push('<ellipse cx="' + dc[0].toFixed(1) + '" cy="' + dc[1].toFixed(1) + '" rx="62" ry="26" fill="#e7e5e4"/>');
parts.push('<ellipse cx="' + dc[0].toFixed(1) + '" cy="' + (dc[1] - 4).toFixed(1) + '" rx="54" ry="20" fill="#d6d3d1"/>');
parts.push('<ellipse cx="' + dc[0].toFixed(1) + '" cy="' + (dc[1] - 7).toFixed(1) + '" rx="40" ry="13" fill="#a8a29e" opacity="0.5"/>');
parts.push('</g>');
parts.push('<path d="M' + dc[0].toFixed(1) + ' ' + (dc[1] - 10).toFixed(1) + ' L' + (dc[0] - 30).toFixed(1) + ' ' + (dc[1] - 52).toFixed(1) + '" stroke="#78716c" stroke-width="3" fill="none"/>');
parts.push(box((dc[0] - 37), (dc[1] - 62), 0, 14, 10, 12, '#3f3f46').replace(/translate/g, 'translate'));
parts.push(led(dc[0] - 30, 0, dc[1] - 56, '#ef4444', 1.6, 0));
for (let i = 0; i < 3; i++) {
  parts.push('<path d="M' + (dc[0] - 44 - i * 16) + ' ' + (dc[1] - 70 - i * 14) + ' q14 -10 26 -2" stroke="' + accent + '" stroke-width="2" fill="none" opacity="0"><animate attributeName="opacity" values="0;0.8;0" dur="2.2s" begin="' + (i * 0.55) + 's" repeatCount="indefinite"/></path>');
}

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0a0d18"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
