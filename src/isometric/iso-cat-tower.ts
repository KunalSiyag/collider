export interface IsoCatTowerOptions {
  accent?: string;
}
...
export function createIsoCatTower(options: IsoCatTowerOptions = {}): string {
  const { accent = '#f472b6' } = options;
  const rand = rng(73062279);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="96" rx="70" ry="18" fill="#000" opacity="0.35"/>');
parts.push(box(-38, -26, 0, 76, 52, 10, '#78716c'));
parts.push(box(8, -20, 10, 36, 32, 32, accent));
const ent = project(26, -4, 26);
parts.push('<circle cx="' + ent[0].toFixed(1) + '" cy="' + ent[1].toFixed(1) + '" r="10" fill="#1c1917"/>');
parts.push(cyl(-12, -2, 10, 9, 54, '#a16207', 8));
parts.push('<path d="M-20 20 L-6 34 M-20 40 L-6 54 M-20 60 L-6 74"? stroke="#7c4a21" stroke-width="2" fill="none"/>'.replace('?', ''));
parts.push(box(-30, -22, 64, 52, 44, 8, shade(accent, 1.1)));
parts.push(box(-18, -16, 90, 40, 34, 8, '#78716c'));
parts.push(orb(0, 0, 102, 13, '#fde68a'));
parts.push('<g><animateTransform attributeName="transform" type="rotate" values="-14 -34 -100;14 -34 -100;-14 -34 -100" dur="3.2s" repeatCount="indefinite"/><path d="M-34 -98 L-34 -60" stroke="#d6d3d1" stroke-width="1.5" fill="none"/><circle cx="-34" cy="-54" r="6" fill="' + shade(accent, 1.3) + '"/></g>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#151119"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
