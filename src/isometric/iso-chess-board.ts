export interface IsoChessBoardOptions {
  accent?: string;
}
...
export function createIsoChessBoard(options: IsoChessBoardOptions = {}): string {
  const { accent = '#e7e5e4' } = options;
  const rand = rng(1570872410);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="105" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-56, -56, 0, 112, 112, 8, '#44403c'));
for (let i = 0; i < 8; i++) for (let j = 0; j < 8; j++) parts.push(box(-56 + i * 14, -56 + j * 14, 8, 14, 14, 2.5, (i + j) % 2 ? '#e7e5e4' : '#1c1917'));
function piece(x, y, kind, col) {
  if (kind === 0) { return pyr(x, y, 10.5, 10, 10, 12, col) + orb(x + 5, y + 5, 25, 4.5, col); }
  if (kind === 1) { return box(x, y, 10.5, 11, 11, 18, col) + box(x + 1.5, y + 1.5, 28.5, 8, 8, 4, col); }
  return pyr(x, y, 10.5, 11, 11, 20, col) + orb(x + 5.5, y + 5.5, 34, 5, col);
}
parts.push(piece(-42, 28, 0, '#e7e5e4'));
parts.push(piece(-14, 28, 1, '#e7e5e4'));
parts.push(piece(14, 28, 0, '#e7e5e4'));
parts.push(piece(42, 28, 2, '#e7e5e4'));
parts.push(piece(-42, -42, 2, '#27272a'));
parts.push(piece(-14, -14, 1, '#27272a'));
parts.push(piece(14, -14, 0, '#27272a'));
parts.push(box(30, 4, 10.5, 12, 12, 4, '#27272a'));
