export interface IsoDiceTowerOptions {
  accent?: string;
}
...
export function createIsoDiceTower(options: IsoDiceTowerOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(3408395330);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="76" rx="95" ry="24" fill="#000" opacity="0.35"/>');
parts.push(box(-56, -34, 0, 112, 68, 10, '#6d4c41'));
parts.push(box(-30, -30, 10, 60, 60, 96, '#7c4a21'));
parts.push('<path d="' + poly([[-30, 30, 30], [30, 30, 30], [30, 30, 44], [-30, 30, 44]]) + '" fill="#1c1917"/>');
parts.push('<path d="' + poly([[-30, 30, 66], [30, 30, 66], [30, 30, 80], [-30, 30, 80]]) + '" fill="#1c1917"/>');
parts.push(box(-34, -34, 106, 68, 68, 8, '#5d4037'));
parts.push(pyr(-38, -38, 114, 76, 76, 12, '#8d5a2b'));
parts.push(box(-16, 6, 10, 20, 20, 20, '#fafaf9'));
function pips(x, y, z) {
  const c = project(x, y, z);
  let s = '';
  s += '<circle cx="' + (c[0] - 4).toFixed(1) + '" cy="' + (c[1] - 2).toFixed(1) + '" r="1.8" fill="#1c1917"/>';
  s += '<circle cx="' + (c[0] + 4).toFixed(1) + '" cy="' + (c[1] + 2).toFixed(1) + '" r="1.8" fill="#1c1917"/>';
  s += '<circle cx="' + c[0].toFixed(1) + '" cy="' + c[1].toFixed(1) + '" r="1.8" fill="#1c1917"/>';
  return s;
}
parts.push(pips(-6, 16, 30));
parts.push(box(14, 22, 10, 16, 16, 16, '#fafaf9'));
parts.push(pips(22, 30, 26));
parts.push(box(-44, 24, 10, 14, 14, 14, '#fef3c7'));
