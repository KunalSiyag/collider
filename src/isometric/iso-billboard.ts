export interface IsoBillboardOptions {
  accent?: string;
}
...
export function createIsoBillboard(options: IsoBillboardOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(4275057840);
  const parts: string[] = [];

parts.push(box(-32, -8, 0, 10, 16, 74, '#52525b'));
parts.push(box(22, -8, 0, 10, 16, 74, '#52525b'));
parts.push(box(-58, -14, 74, 116, 28, 58, '#18181b'));
parts.push('<path d="' + poly([[-52, 13.5, 80], [52, 13.5, 80], [52, 13.5, 126], [-52, 13.5, 126]]) + '" fill="#020617"/>');
const cp = project(-30, 13.5, 106);
parts.push('<circle cx="' + cp[0].toFixed(1) + '" cy="' + cp[1].toFixed(1) + '" r="14" fill="' + accent + '"><animate attributeName="r" values="14;16;14" dur="2.4s" repeatCount="indefinite"/></circle>');
for (let i = 0; i < 4; i++) parts.push('<path d="' + poly([[2 + i * 12, 13.4, 88 + i * 9], [10 + i * 12, 13.4, 88 + i * 9], [10 + i * 12, 13.4, 93 + i * 9], [2 + i * 12, 13.4, 93 + i * 9]]) + '" fill="#f472b6"/>');
for (let i = 0; i < 8; i++) parts.push(led(-52 + i * 14.8, 13.5, 127, i % 2 ? accent : '#f472b6', 1.1, i * 0.14));
parts.push(box(-50, -6, 0, 8, 8, 10, '#3f3f46'));
parts.push(box(42, -6, 0, 8, 8, 10, '#3f3f46'));
parts.push('<polygon points="-46,-52 -110,-96 -110,-30" fill="' + accent + '" opacity="0.08"/>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#10101e"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
