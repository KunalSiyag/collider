export interface IsoLighthouseOptions {
  accent?: string;
}
...
export function createIsoLighthouse(options: IsoLighthouseOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(2606691805);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="10" rx="118" ry="34" fill="#164e63"/>');
parts.push('<path d="M-100 18 Q-40 30 30 20" stroke="#22d3ee" stroke-width="2" fill="none" opacity="0.5"><animate attributeName="opacity" values="0.5;0.15;0.5" dur="3s" repeatCount="indefinite"/></path>');
parts.push(box(-46, -28, 0, 92, 56, 16, '#44403c'));
parts.push(box(-27, -17, 16, 54, 34, 27, '#e7e5e4'));
parts.push(box(-23, -15, 43, 46, 30, 27, accent));
parts.push(box(-19, -13, 70, 38, 26, 27, '#e7e5e4'));
parts.push(box(-15, -11, 97, 30, 22, 27, accent));
parts.push(box(-17, -12, 124, 34, 24, 8, '#27272a'));
parts.push(box(-11, -8, 132, 22, 16, 15, '#fde68a'));
parts.push(pyr(-13, -10, 147, 26, 20, 16, '#7f1d1d'));
parts.push(led(0, 0, 140, '#fbbf24', 1.6, 0));
parts.push('<polygon points="-18,-146 -160,-196 -160,-120" fill="#fde68a"><animate attributeName="opacity" values="0;0.28;0" dur="3s" repeatCount="indefinite"/></polygon>');
parts.push('<polygon points="16,-142 158,-186 158,-114" fill="#fde68a"><animate attributeName="opacity" values="0.28;0;0.28" dur="3s" repeatCount="indefinite"/></polygon>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0d1524"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
