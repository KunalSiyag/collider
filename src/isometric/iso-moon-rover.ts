export interface IsoMoonRoverOptions {
  accent?: string;
}
...
export function createIsoMoonRover(options: IsoMoonRoverOptions = {}): string {
  const { accent = '#4ade80' } = options;
  const rand = rng(2001779225);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="125" ry="34" fill="#52525b" opacity="0.35"/>');
for (let i = 0; i < 4; i++) parts.push('<ellipse cx="' + (-100 + i * 55) + '" cy="' + (20 + i * 12) + '" rx="' + (10 + i * 4) + '" ry="' + (4 + i * 1.5) + '" fill="#27272a" opacity="0.6"/>');
parts.push('<path d="M-120 66 q10 -3 20 0 M-100 72 q10 -3 20 0" stroke="#71717a" stroke-width="2" fill="none" stroke-dasharray="3 4"/>');
parts.push(box(-40, -22, 26, 80, 44, 14, '#d4d4d8'));
parts.push(box(-30, -16, 40, 52, 32, 16, '#a1a1aa'));
parts.push(box(-26, -12, 56, 44, 24, 4, '#1e3a8a'));
for (let i = 0; i < 3; i++) parts.push('<path d="' + poly([[-24 + i * 16, -12, 57], [-12 + i * 16, -12, 57]]) + '" stroke="#3b82f6" stroke-width="1.5" fill="none"/>');
function wheel(x, y) {
  const p = project(x, y, 14);
  return '<circle cx="' + p[0].toFixed(1) + '" cy="' + p[1].toFixed(1) + '" r="12" fill="#18181b"/><circle cx="' + p[0].toFixed(1) + '" cy="' + p[1].toFixed(1) + '" r="5" fill="#52525b"/>';
}
parts.push(wheel(-30, 22));
parts.push(wheel(0, 22));
parts.push(wheel(30, 22));
parts.push(wheel(-30, -22));
parts.push(wheel(30, -22));
parts.push('<path d="M44 -6 L44 -52" stroke="#71717a" stroke-width="3" fill="none"/>');
parts.push(orb(44, -6, 74, 9, '#e2e8f0'));
parts.push(led(-34, -14, 62, accent, 1.6, 0));
parts.push('<circle cx="120" cy="-190" r="14" fill="#38bdf8" opacity="0.9"/><circle cx="116" cy="-194" r="4" fill="#bae6fd" opacity="0.7"/>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0b0c12"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
