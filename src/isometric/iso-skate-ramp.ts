export interface IsoSkateRampOptions {
  accent?: string;
}
...
export function createIsoSkateRamp(options: IsoSkateRampOptions = {}): string {
  const { accent = '#f472b6' } = options;
  const rand = rng(2715834088);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="80" rx="125" ry="28" fill="#000" opacity="0.35"/>');
for (let s = 0; s < 4; s++) {
  const w = 26 - s * 5;
  const z = s * 16;
  parts.push(box(-88, -26, z, w, 52, 16, '#52525b'));
  parts.push(box(62, -26, z, w, 52, 16, '#57534e'));
}
parts.push(box(-62, -26, 0, 124, 52, 16, '#3f3f46'));
parts.push(box(-90, -28, 80, 30, 56, 4, '#a8a29e'));
parts.push(box(60, -28, 80, 30, 56, 4, '#a8a29e'));
for (let i = 0; i < 3; i++) parts.push('<path d="' + poly([[-62, 26.5, 20 + i * 18], [62, 26.5, 20 + i * 18]]) + '" stroke="#18181b" stroke-width="2" fill="none"/>');
parts.push('<circle cx="' + project(-40, 26.5, 40)[0].toFixed(1) + '" cy="' + project(-40, 26.5, 40)[1].toFixed(1) + '" r="9" fill="' + accent + '" opacity="0.85"/>');
parts.push('<circle cx="' + project(20, 26.5, 56)[0].toFixed(1) + '" cy="' + project(20, 26.5, 56)[1].toFixed(1) + '" r="6" fill="#22d3ee" opacity="0.8"/>');
let sk8 = box(-14, -6, 96, 30, 12, 3, '#b45309') + orb(-8, -2, 94, 3.5, '#e7e5e4') + orb(12, 4, 94, 3.5, '#e7e5e4');
parts.push('<g><animateTransform attributeName="transform" type="translate" values="0 0;0 -16;0 0" keyTimes="0;0.5;1" dur="1.6s" repeatCount="indefinite"/>' + sk8 + '</g>');
parts.push('<ellipse cx="0" cy="98" rx="20" ry="6" fill="#000" opacity="0.3"><animate attributeName="rx" values="20;14;20" dur="1.6s" repeatCount="indefinite"/></ellipse>');
parts.push(led(-70, 30, 84, '#fbbf24', 2.2, 0));
parts.push(led(66, 30, 84, '#fbbf24', 2.2, 1.1));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#131018"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
