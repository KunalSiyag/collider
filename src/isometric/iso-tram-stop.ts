export interface IsoTramStopOptions {
  accent?: string;
}
...
export function createIsoTramStop(options: IsoTramStopOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(2720014640);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="120" ry="30" fill="#000" opacity="0.35"/>');
parts.push(box(-100, 34, 0, 200, 18, 4, '#27272a'));
parts.push(box(-104, -26, 0, 208, 60, 6, '#3f3f46'));
for (let i = 0; i < 9; i++) parts.push(box(-98 + i * 23, -22, 6, 12, 52, 2, '#71717a'));
parts.push(box(-56, -22, 6, 112, 3, 62, '#0e7490'));
parts.push(box(-58, -24, 68, 116, 64, 5, '#18181b'));
parts.push('<path d="M-58 -30 Q0 -46 58 -30 L58 -24 L-58 -24 Z" fill="' + shade(accent, 0.8) + '"/>');
parts.push(box(-48, -16, 6, 10, 10, 66, '#52525b'));
parts.push(box(44, -16, 6, 10, 10, 66, '#52525b'));
parts.push(box(-40, -14, 6, 76, 18, 9, '#7f1d1d'));
parts.push(box(-36, -10, 15, 20, 14, 42, '#18181b'));
parts.push('<path d="' + poly([[-33, 3.5, 20], [-19, 3.5, 20], [-19, 3.5, 50], [-33, 3.5, 50]]) + '" fill="#0ea5e9" opacity="0.75"><animate attributeName="opacity" values="0.75;0.4;0.75" dur="2.2s" repeatCount="indefinite"/></path>');
parts.push(box(10, -12, 15, 30, 12, 8, '#18181b'));
parts.push('<path d="' + poly([[13, -11.8, 18], [37, -11.8, 18]]) + '" stroke="#ef4444" stroke-width="3" fill="none"/>');
parts.push('<path d="' + poly([[13, -11.8, 15], [30, -11.8, 15]]) + '" stroke="#fbbf24" stroke-width="3" fill="none"/>');
parts.push(led(46, -18, 74, accent, 1.8, 0));
