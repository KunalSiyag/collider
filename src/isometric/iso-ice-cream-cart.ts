export interface IsoIceCreamCartOptions {
  accent?: string;
}
...
export function createIsoIceCreamCart(options: IsoIceCreamCartOptions = {}): string {
  const { accent = '#f9a8d4' } = options;
  const rand = rng(2179339660);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="66" rx="100" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-46, -22, 0, 92, 44, 40, shade(accent, 1.05)));
parts.push(box(-50, -26, 40, 100, 52, 8, '#fafaf9'));
parts.push(box(-30, -20, 48, 56, 34, 16, '#e0f2fe'));
for (let i = 0; i < 3; i++) parts.push(puff(-20 + i * 18, -4, 66, 5, 2.6 + i * 0.4, i * 0.8, '#f0f9ff'));
parts.push(box(-44, -24, 48, 88, 48, 4, accent));
for (let i = 0; i < 3; i++) parts.push(box(-40 + i * 30, -18, 52, 8, 8, 3, '#a16207'));
parts.push(pyr(-38 + 0, -14, 55, 9, 9, 12, '#d9a066'));
parts.push(orb(-33.5, -9.5, 69, 6.5, '#fbcfe8'));
parts.push(pyr(-24, -12, 55, 9, 9, 12, '#d9a066'));
parts.push(orb(-19.5, -7.5, 69, 6.5, '#a7f3d0'));
const wc = project(-24, 22, 12);
const wd = project(24, 22, 12);
parts.push('<circle cx="' + wc[0].toFixed(1) + '" cy="' + wc[1].toFixed(1) + '" r="11" fill="#18181b"/><circle cx="' + wc[0].toFixed(1) + '" cy="' + wc[1].toFixed(1) + '" r="4" fill="#f9a8d4"/>');
parts.push('<circle cx="' + wd[0].toFixed(1) + '" cy="' + wd[1].toFixed(1) + '" r="11" fill="#18181b"/><circle cx="' + wd[0].toFixed(1) + '" cy="' + wd[1].toFixed(1) + '" r="4" fill="#f9a8d4"/>');
parts.push(led(40, -22, 54, '#22d3ee', 2, 0));
