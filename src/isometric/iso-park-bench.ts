export interface IsoParkBenchOptions {
  accent?: string;
}
...
export function createIsoParkBench(options: IsoParkBenchOptions = {}): string {
  const { accent = '#a16207' } = options;
  const rand = rng(2859328558);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="14" rx="105" ry="30" fill="#14532d" opacity="0.55"/>');
parts.push(box(-42, -12, 0, 8, 26, 20, '#3f3f46'));
parts.push(box(34, -12, 0, 8, 26, 20, '#3f3f46'));
for (let j = 0; j < 3; j++) parts.push(box(-46, -14 + j * 11, 20, 92, 9, 5, shade(accent, 1 + j * 0.08)));
for (let j = 0; j < 3; j++) parts.push(box(-46, -18, 28 + j * 13, 92, 6, 8, shade(accent, 1.05 + j * 0.06)));
parts.push(cyl(74, 14, 0, 4, 78, '#27272a', 6));
parts.push(orb(74, 14, 82, 9, '#fde68a'));
parts.push('<circle cx="52" cy="-96" r="26" fill="#fde68a" opacity="0.12"><animate attributeName="r" values="26;31;26" dur="3.4s" repeatCount="indefinite"/></circle>');
parts.push('<circle cx="-30" cy="-40" r="3" fill="#4ade80"><animate attributeName="cy" values="-40;40;-40" dur="4.2s" repeatCount="indefinite"/><animate attributeName="opacity" values="0.9;0;0.9" dur="4.2s" repeatCount="indefinite"/></circle>');
parts.push('<circle cx="20" cy="-30" r="2.4" fill="#a3e635"><animate attributeName="cy" values="-30;50;-30" dur="5.1s" begin="1.3s" repeatCount="indefinite"/><animate attributeName="opacity" values="0.8;0;0.8" dur="5.1s" begin="1.3s" repeatCount="indefinite"/></circle>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#121a12"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
    '<rect width="320" height="320" fill="url(#bg)"/>' +
    '<g transform="translate(160 215)">' +
    '<circle r="106" fill="#fbbf24" opacity="0.08"/>' +
    parts.join('') +
    '</g></svg>';
}
