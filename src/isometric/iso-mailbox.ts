export interface IsoMailboxOptions {
  accent?: string;
}
...
export function createIsoMailbox(options: IsoMailboxOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(3054065327);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="80" rx="80" ry="20" fill="#14532d" opacity="0.5"/>');
parts.push(box(-6, -5, 0, 12, 10, 56, '#52525b'));
function mbox(x, col) {
  let s = box(x, -14, 56, 30, 26, 20, col);
  const fc = project(x + 15, -1, 66);
  s += '<ellipse cx="' + fc[0].toFixed(1) + '" cy="' + fc[1].toFixed(1) + '" rx="11" ry="9" fill="' + shade(col, 0.7) + '"/><rect x="' + (fc[0] - 6).toFixed(1) + '" y="' + (fc[1] - 2).toFixed(1) + '" width="12" height="4" rx="2" fill="#292524"/>';
  return s;
}
parts.push(mbox(-52, '#3b82f6'));
parts.push(mbox(-16, '#e7e5e4'));
parts.push(mbox(20, '#3b82f6'));
const fl = project(-37, -1, 76);
parts.push('<g><animateTransform attributeName="transform" type="translate" values="0 0;0 -10;0 -10;0 0" keyTimes="0;0.15;0.8;1" dur="4s" repeatCount="indefinite"/><rect x="' + (fl[0] - 2).toFixed(1) + '" y="' + fl[1].toFixed(1) + '" width="4" height="14" fill="#78350f"/><rect x="' + (fl[0] + 1).toFixed(1) + '" y="' + (fl[1] - 6).toFixed(1) + '" width="9" height="6" fill="' + accent + '"/></g>');
parts.push(box(-12, 2, 62, 10, 4, 3, '#fefce8'));
parts.push(orb(60, 30, 4, 3, '#4ade80'));
parts.push(orb(66, 36, 4, 2.6, '#4ade80'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#111410"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
