export interface IsoDrumKitOptions {
  accent?: string;
}
...
export function createIsoDrumKit(options: IsoDrumKitOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(3583503712);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="90" rx="115" ry="28" fill="#000" opacity="0.35"/>');
parts.push(cyl(-26, 6, 0, 34, 44, accent, 12));
const kc = project(-26, 6, 24);
parts.push('<ellipse cx="' + kc[0].toFixed(1) + '" cy="' + kc[1].toFixed(1) + '" rx="30" ry="15" fill="#fafaf9"/><ellipse cx="' + kc[0].toFixed(1) + '" cy="' + kc[1].toFixed(1) + '" rx="24" ry="12" fill="none" stroke="' + accent + '" stroke-width="2"/>');
parts.push(cyl(24, -18, 34, 17, 14, shade(accent, 0.9), 10));
parts.push(cyl(-4, -34, 52, 14, 12, accent, 10));
parts.push(cyl(-40, -30, 52, 14, 12, accent, 10));
function stand(x, y, topZ) {
  return '<path d="M' + project(x, y, 0)[0].toFixed(1) + ' ' + project(x, y, 0)[1].toFixed(1) + ' L' + project(x, y, topZ)[0].toFixed(1) + ' ' + project(x, y, topZ)[1].toFixed(1) + '" stroke="#a1a1aa" stroke-width="3" fill="none"/>' + cyl(x, y, topZ, 15, 2, '#fbbf24', 10);
}
parts.push(stand(52, 8, 74));
parts.push(stand(-58, -6, 88));
parts.push(cyl(0, -52, 66, 13, 10, shade(accent, 0.95), 10));
parts.push(box(-6, -60, 0, 14, 14, 52, '#52525b'));
parts.push('<path d="M' + project(-30, 10, 46)[0].toFixed(1) + ' ' + project(-30, 10, 46)[1].toFixed(1) + ' L' + project(-14, -2, 50)[0].toFixed(1) + ' ' + project(-14, -2, 50)[1].toFixed(1) + '" stroke="#d6d3d1" stroke-width="2.5" fill="none"/>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#131019"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
