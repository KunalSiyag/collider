export interface IsoFarmSiloOptions {
  accent?: string;
}
...
export function createIsoFarmSilo(options: IsoFarmSiloOptions = {}): string {
  const { accent = '#a3a3a3' } = options;
  const rand = rng(2179365121);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="86" rx="120" ry="30" fill="#3f6212" opacity="0.5"/>');
parts.push(cyl(-30, -14, 0, 34, 130, '#94a3b8', 10));
for (let i = 0; i < 6; i++) {
  const p1 = project(-64 + Math.cos(0.6) * 0, 0, 0);
  parts.push('<path d="' + poly([[-64, 20, 12 + i * 21], [4, 20, 12 + i * 21]]) + '" stroke="#cbd5e1" stroke-width="2" opacity="0.6" fill="none"/>');
}
parts.push(orb(-30, -14, 138, 33, shade('#94a3b8', 1.15)));
parts.push(box(28, -20, 0, 70, 46, 54, '#7c2d12'));
parts.push(pyr(24, -26, 54, 78, 58, 30, '#dc2626'));
parts.push('<path d="' + poly([[32, 26, 16], [52, 26, 16], [52, 26, 40], [32, 26, 40]]) + '" fill="#431407"/>');
for (let i = 0; i < 4; i++) parts.push('<path d="' + poly([[32, 26, 8 + i * 11], [96, 26, 8 + i * 11]]) + '" stroke="#5b1d09" stroke-width="2" fill="none"/>');
parts.push(box(38, -14, 54, 26, 22, 18, '#e3b341'));
parts.push(box(72, 6, 54, 26, 22, 18, '#d3a136'));
parts.push(led(60, -22, 60, '#22c55e', 2.6, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#12140e"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
