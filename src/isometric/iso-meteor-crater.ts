export interface IsoMeteorCraterOptions {
  accent?: string;
}
...
export function createIsoMeteorCrater(options: IsoMeteorCraterOptions = {}): string {
  const { accent = '#f97316' } = options;
  const rand = rng(3151427267);
  const parts: string[] = [];

for (let i = 0; i < 12; i++) {
  const sx = rand() * 300 - 150;
  const sy = -rand() * 200 - 40;
  parts.push('<circle cx="' + sx.toFixed(0) + '" cy="' + sy.toFixed(0) + '" r="1.5" fill="#e2e8f0"><animate attributeName="opacity" values="0.2;1;0.2" dur="' + (1.5 + rand() * 2).toFixed(1) + 's" begin="' + (rand() * 2).toFixed(1) + 's" repeatCount="indefinite"/></circle>');
}
parts.push('<ellipse cx="0" cy="50" rx="120" ry="36" fill="#44403c"/>');
parts.push('<ellipse cx="0" cy="54" rx="84" ry="24" fill="#292524"/>');
parts.push('<ellipse cx="0" cy="56" rx="52" ry="15" fill="#1c1917"/>');
parts.push(pyr(-56, -20, 42, 22, 16, 14, '#57534e'));
parts.push(pyr(38, -8, 44, 18, 14, 11, '#57534e'));
parts.push(pyr(-10, 30, 44, 16, 12, 9, '#78716c'));
parts.push(orb(0, 0, 52, 13, shade(accent, 0.9)));
parts.push(orb(-6, -4, 58, 5, '#fbbf24'));
parts.push('<circle cx="' + project(0, 0, 52)[0].toFixed(1) + '" cy="' + project(0, 0, 52)[1].toFixed(1) + '" r="17" fill="none" stroke="' + accent + '" stroke-width="2" opacity="0.4"><animate attributeName="r" values="17;26;17" dur="2.4s" repeatCount="indefinite"/><animate attributeName="opacity" values="0.4;0;0.4" dur="2.4s" repeatCount="indefinite"/></circle>');
parts.push(puff(4, 4, 62, 8, 3, 0.6, '#a8a29e'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0c0c16"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
