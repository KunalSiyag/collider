export interface IsoBarbellOptions {
  accent?: string;
}
...
export function createIsoBarbell(options: IsoBarbellOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(884913723);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="90" rx="110" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-26, -16, 0, 14, 32, 34, '#3f3f46'));
parts.push(box(12, -16, 0, 14, 32, 34, '#3f3f46'));
parts.push(box(-86, -7, 34, 172, 14, 10, '#a1a1aa'));
parts.push(box(-72, -11, 32, 8, 22, 14, '#52525b'));
parts.push(box(64, -11, 32, 8, 22, 14, '#52525b'));
parts.push(box(-100, -23, 14, 11, 46, 50, accent));
parts.push(box(-88, -19, 17, 9, 38, 45, '#8b5cf6'));
parts.push(box(77, -23, 14, 11, 46, 50, accent));
parts.push(box(67, -19, 17, 9, 38, 45, '#8b5cf6'));
parts.push('<circle cx="-104" cy="86" r="4" fill="#fafafa" opacity="0.5"/>');
parts.push('<circle cx="108" cy="92" r="3" fill="#fafafa" opacity="0.4"/>');
parts.push(led(0, -14, 40, '#fbbf24', 1.8, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#10151a"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
