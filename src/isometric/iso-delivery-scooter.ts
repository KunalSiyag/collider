export interface IsoDeliveryScooterOptions {
  accent?: string;
}
...
export function createIsoDeliveryScooter(options: IsoDeliveryScooterOptions = {}): string {
  const { accent = '#22d3ee' } = options;
  const rand = rng(976614003);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="60" rx="105" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-90, 24, 0, 180, 22, 5, '#27272a'));
for (let i = 0; i < 6; i++) parts.push(box(-82 + i * 30, 30, 5, 14, 5, 2, '#fbbf24'));
const wf = project(-34, 0, 13);
const wb = project(40, 0, 13);
parts.push('<circle cx="' + wf[0].toFixed(1) + '" cy="' + wf[1].toFixed(1) + '" r="13" fill="#18181b"/><circle cx="' + wf[0].toFixed(1) + '" cy="' + wf[1].toFixed(1) + '" r="5" fill="#52525b"/>');
parts.push('<circle cx="' + wb[0].toFixed(1) + '" cy="' + wb[1].toFixed(1) + '" r="13" fill="#18181b"/><circle cx="' + wb[0].toFixed(1) + '" cy="' + wb[1].toFixed(1) + '" r="5" fill="#52525b"/>');
parts.push(box(-24, -7, 16, 62, 14, 6, accent));
parts.push(box(-6, -8, 22, 30, 12, 12, '#e7e5e4'));
parts.push('<path d="M-34 16 L-42 60 M-42 60 L-46 44 M-42 60 L-30 48" stroke="#71717a" stroke-width="4" stroke-linecap="round" fill="none"/>');
parts.push('<path d="M-44 64 L-38 44 M-38 44 L-38 36" stroke="#52525b" stroke-width="4" stroke-linecap="round" fill="none"/>');
parts.push(box(28, -10, 24, 24, 20, 24, shade(accent, 0.9)));
parts.push('<circle cx="' + project(40, 0, 34)[0].toFixed(1) + '" cy="' + project(40, 0, 34)[1].toFixed(1) + '" r="4" fill="' + accent + '"/>');
parts.push('<polygon points="' + [project(44, 0, 34), project(120, -20, 44), project(120, 16, 44)].map((p) => p[0].toFixed(1) + ',' + p[1].toFixed(1)).join(' ') + '" fill="' + accent + '" opacity="0.12"><animate attributeName="opacity" values="0.12;0.05;0.12" dur="2s" repeatCount="indefinite"/></polygon>');
parts.push(orb(-14, -12, 40, 6, '#ef4444'));
