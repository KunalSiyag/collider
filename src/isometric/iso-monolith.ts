export interface IsoMonolithOptions {
  accent?: string;
}
...
export function createIsoMonolith(options: IsoMonolithOptions = {}): string {
  const { accent = '#8b5cf6' } = options;
  const rand = rng(1786109065);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="110" ry="28" fill="#292524" opacity="0.8"/>');
parts.push(box(-16, -14, 0, 32, 28, 168, '#0c0a09'));
parts.push('<path d="' + poly([[-16, 14, 6], [-16, 14, 162]]) + '" stroke="#27272a" stroke-width="2" fill="none"/>');
parts.push('<path d="' + poly([[-4, -14.5, 12], [-4, -14.5, 156]]) + '" stroke="' + accent + '" stroke-width="2.5" fill="none"><animate attributeName="opacity" values="1;0.25;1" dur="2.8s" repeatCount="indefinite"/></path>');
parts.push(pyr(-52, 8, 0, 20, 18, 12, '#44403c'));
parts.push(pyr(34, 18, 0, 16, 14, 10, '#57534e'));
parts.push(pyr(44, -16, 0, 12, 12, 8, '#3f3f46'));
parts.push('<g><animateTransform attributeName="transform" type="translate" values="0 0;0 -8;0 0" dur="4s" repeatCount="indefinite"/>' + box(40, -40, 120, 10, 10, 10, '#57534e') + '</g>');
parts.push('<g><animateTransform attributeName="transform" type="translate" values="0 -5;0 5;0 -5" dur="5s" repeatCount="indefinite"/>' + box(-52, -30, 100, 8, 8, 8, '#78716c') + '</g>');
parts.push('<circle cx="90" cy="-160" r="2" fill="#c4b5fd"><animate attributeName="opacity" values="1;0.2;1" dur="2.2s" repeatCount="indefinite"/></circle>');
parts.push('<circle cx="108" cy="-130" r="1.6" fill="#e9d5ff"><animate attributeName="opacity" values="0.3;1;0.3" dur="3.1s" repeatCount="indefinite"/></circle>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0d0d18"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
