export interface IsoHammockOptions {
  accent?: string;
}
...
export function createIsoHammock(options: IsoHammockOptions = {}): string {
  const { accent = '#f59e0b' } = options;
  const rand = rng(3823262359);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="120" ry="30" fill="#3f6212" opacity="0.35"/>');
function palm(x, y, h) {
  let s = box(x - 5, y - 5, 0, 10, 10, h, '#8d5a2b');
  for (let i = 0; i < 5; i++) {
    const a = i / 5 * Math.PI * 2;
    s += '<path d="M' + project(x, y, h)[0].toFixed(1) + ' ' + project(x, y, h)[1].toFixed(1) + ' q' + (Math.cos(a) * 34).toFixed(0) + ' ' + (Math.sin(a) * 16 - 14).toFixed(0) + ' ' + (Math.cos(a) * 52).toFixed(0) + ' ' + (Math.sin(a) * 24 + 6).toFixed(0) + '" stroke="#15803d" stroke-width="5" fill="none" stroke-linecap="round"/>';
  }
  return s;
}
parts.push(palm(-62, -18, 96));
parts.push(palm(66, 20, 104));
parts.push('<g><animateTransform attributeName="transform" type="rotate" values="-3 -62 -100;3 -62 -100;-3 -62 -100" dur="4.4s" repeatCount="indefinite"/>');
parts.push('<path d="M-58 -92 Q0 -46 62 -98" stroke="' + accent + '" stroke-width="9" fill="none" stroke-linecap="round"/>');
parts.push('<path d="M-56 -88 Q0 -44 60 -94" stroke="#fef3c7" stroke-width="3" fill="none" stroke-dasharray="7 9"/>');
parts.push('</g>');
parts.push(orb(-30, -40, -66, 8, shade(accent, 1.25)));
parts.push('<circle cx="-90" cy="-130" r="3" fill="#a3e635"><animate attributeName="cy" values="-130;-60;-130" dur="6s" repeatCount="indefinite"/><animate attributeName="opacity" values="0;0.9;0" dur="6s" repeatCount="indefinite"/></circle>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0f1412"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
