export interface IsoBookshelfOptions {
  accent?: string;
}
...
export function createIsoBookshelf(options: IsoBookshelfOptions = {}): string {
  const { accent = '#8b5cf6' } = options;
  const rand = rng(2235082960);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="112" rx="80" ry="22" fill="#000" opacity="0.35"/>');
parts.push(box(-48, -20, 0, 8, 40, 130, '#5d4037'));
parts.push(box(40, -20, 0, 8, 40, 130, '#5d4037'));
parts.push(box(-48, -20, 126, 96, 40, 8, '#6d4c41'));
const palette = [accent, '#22d3ee', '#fbbf24', '#4ade80', '#f472b6'];
for (let s = 0; s < 4; s++) {
  const z = 4 + s * 31;
  parts.push(box(-44, -18, z, 88, 36, 5, '#7c4a21'));
  if (s === 2) continue;
  let bx = -40;
  while (bx < 24) {
    const bw = 5 + rand() * 6;
    const bh = 22 + rand() * 14;
    parts.push(box(bx, -14, z + 5, bw, 26, bh, palette[Math.floor(rand() * palette.length)]));
    bx += bw + 1.5;
  }
}
parts.push(cyl(-30, 0, 98, 8, 10, '#b45309', 8));
parts.push(orb(-30, 0, 116, 11, '#15803d'));
parts.push(orb(-22, 8, 122, 7, '#22c55e'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#141019"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
