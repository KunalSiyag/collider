export interface IsoDronePadOptions {
  accent?: string;
}
...
export function createIsoDronePad(options: IsoDronePadOptions = {}): string {
  const { accent = '#4ade80' } = options;
  const rand = rng(3567639637);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="110" ry="28" fill="#000" opacity="0.35"/>');
parts.push(box(-64, -44, 0, 128, 88, 8, '#27272a'));
parts.push('<path d="M-22 6 L22 -38 M22 6 L-22 -38 M-22 -16 L22 -16" stroke="' + accent + '" stroke-width="6" fill="none" opacity="0.9"/>');
for (let i = 0; i < 4; i++) parts.push(led([-58, 58, 58, -58][i], [-38, -38, 38, 38][i], 9, accent, 0.8, i * 0.2));
parts.push('<ellipse cx="0" cy="-64" rx="34" ry="12" fill="#000" opacity="0.3"><animate attributeName="rx" values="30;38;30" dur="2s" repeatCount="indefinite"/></ellipse>');
let drone = '';
drone += box(-20, -14, 96, 40, 28, 14, '#e5e7eb');
drone += box(-8, -6, 110, 16, 12, 6, '#52525b');
drone += led(4, 2, 118, '#ef4444', 0.7, 0);
const arm = box(-34, -22, 104, 12, 8, 5, '#a1a1aa') + box(22, 14, 104, 12, 8, 5, '#a1a1aa') + orb(-30, -18, 112, 3, '#71717a') + orb(28, 16, 112, 3, '#71717a');
drone += '<ellipse cx="-46" cy="-124" rx="16" ry="5" fill="#d4d4d8" opacity="0.55"><animate attributeName="opacity" values="0.55;0.2;0.55" dur="0.15s" repeatCount="indefinite"/></ellipse>';
drone += '<ellipse cx="46" cy="-96" rx="16" ry="5" fill="#d4d4d8" opacity="0.55"><animate attributeName="opacity" values="0.2;0.55;0.2" dur="0.15s" repeatCount="indefinite"/></ellipse>';
parts.push('<g><animateTransform attributeName="transform" type="translate" values="0 0;0 -7;0 0" dur="2.6s" repeatCount="indefinite"/>' + drone + '</g>');
parts.push('<path d="M0 -88 L0 -70" stroke="#d4d4d8" stroke-width="2" fill="none"/>');
parts.push(box(-10, -8, 74, 20, 16, 14, '#b45309'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0f1512"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
    '<rect width="320" height="320" fill="url(#bg)"/>' +
    '<g transform="translate(160 230)">' +
    '<circle r="106" fill="#4ade80" opacity="0.08"/>' +
    parts.join('') +
    '</g></svg>';
}
