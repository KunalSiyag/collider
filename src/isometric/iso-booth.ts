export interface IsoBoothOptions {
  accent?: string;
}
...
export function createIsoBooth(options: IsoBoothOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(1613422971);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="94" rx="56" ry="16" fill="#000" opacity="0.35"/>');
parts.push(box(-27, -21, 0, 54, 42, 6, '#27272a'));
parts.push(box(-23, -17, 6, 46, 34, 86, '#0e7490'));
parts.push('<path d="' + poly([[-23, 17, 6], [23, 17, 6], [23, 17, 92], [-23, 17, 92]]) + '" fill="#ffffff" opacity="0.08"/>');
parts.push(box(-23, -17, 6, 4, 4, 86, '#71717a'));
parts.push(box(19, -17, 6, 4, 4, 86, '#71717a'));
parts.push(box(-23, 13, 6, 4, 4, 86, '#71717a'));
parts.push(box(-27, -21, 92, 54, 42, 6, '#18181b'));
parts.push(box(-24, -24, 98, 48, 8, 14, '#18181b'));
parts.push(led(-12, -20, 106, accent, 2, 0));
parts.push(led(4, -20, 106, '#fbbf24', 2, 0.7));
parts.push(box(-14, 12, 40, 20, 4, 14, '#3f3f46'));
parts.push(led(0, 14, 70, '#fbbf24', 3.5, 0));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#12101c"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
    '<rect width="320" height="320" fill="url(#bg)"/>' +
    '<g transform="translate(160 245)">' +
    '<circle r="106" fill="#ef4444" opacity="0.08"/>' +
    parts.join('') +
    '</g></svg>';
}
