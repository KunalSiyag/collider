export interface IsoGuitarAmpOptions {
  accent?: string;
}
...
export function createIsoGuitarAmp(options: IsoGuitarAmpOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(614808564);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="76" rx="95" ry="24" fill="#000" opacity="0.35"/>');
parts.push(box(-52, -26, 0, 104, 52, 84, '#1c1917'));
parts.push('<path d="' + poly([[-48, 25, 8], [48, 25, 8], [48, 25, 60], [-48, 25, 60]]) + '" fill="#44403c"/>');
for (let i = 0; i < 7; i++) parts.push('<path d="' + poly([[-46, 25.2, 10 + i * 7], [46, 25.2, 10 + i * 7]]) + '" stroke="#292524" stroke-width="2" fill="none"/>');
for (let i = 0; i < 6; i++) parts.push(orb(-42 + i * 16, -14, 72, 4.5, i === 0 ? '#e7e5e4' : '#78716c'));
parts.push(led(44, -14, 72, accent, 2.6, 0));
parts.push('<path d="M60 30 q30 -6 24 22 t-30 18" stroke="#18181b" stroke-width="5" fill="none"/>');
parts.push(box(52, 40, 62, 12, 10, 8, '#a16207'));
