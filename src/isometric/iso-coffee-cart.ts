export interface IsoCoffeeCartOptions {
  accent?: string;
}
...
export function createIsoCoffeeCart(options: IsoCoffeeCartOptions = {}): string {
  const { accent = '#b45309' } = options;
  const rand = rng(1309922558);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="70" rx="105" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-46, -22, 0, 92, 44, 36, '#7c2d12'));
parts.push(box(-50, -26, 36, 100, 52, 7, '#a16207'));
const wc = project(-30, 22, 10);
const wd = project(30, 22, 10);
parts.push('<circle cx="' + wc[0].toFixed(1) + '" cy="' + wc[1].toFixed(1) + '" r="12" fill="#18181b"/><circle cx="' + wc[0].toFixed(1) + '" cy="' + wc[1].toFixed(1) + '" r="4" fill="#78716c"/>');
parts.push('<circle cx="' + wd[0].toFixed(1) + '" cy="' + wd[1].toFixed(1) + '" r="12" fill="#18181b"/><circle cx="' + wd[0].toFixed(1) + '" cy="' + wd[1].toFixed(1) + '" r="4" fill="#78716c"/>');
parts.push('<path d="M6 43 L6 118" stroke="#52525b" stroke-width="4" fill="none"/>');
parts.push(pyr(-44, -32, 116, 100, 64, 20, shade(accent, 1.05)));
for (let i = 0; i < 7; i++) parts.push(orb(-38 + i * 13, 26, 137, 3.2, i % 2 ? '#fef3c7' : accent));
parts.push(box(24, -14, 43, 12, 12, 10, '#fafaf9'));
parts.push(box(40, -8, 43, 12, 12, 10, '#fafaf9'));
parts.push(puff(30, -8, 56, 5, 2.4, 0));
parts.push(puff(46, -2, 56, 5, 2.8, 0.9));
parts.push(box(-38, 16, 43, 22, 3, 26, '#1c1917'));
parts.push('<path d="' + poly([[-34, 17.5, 48], [-20, 17.5, 48], [-20, 17.5, 51], [-34, 17.5, 51]]) + '" fill="#fef3c7"/>');
parts.push('<path d="' + poly([[-34, 17.5, 56], [-24, 17.5, 56], [-24, 17.5, 59], [-34, 17.5, 59]]) + '" fill="#fca5a5"/>');

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#161009"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
