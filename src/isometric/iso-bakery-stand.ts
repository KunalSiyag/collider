export interface IsoBakeryStandOptions {
  accent?: string;
}
...
export function createIsoBakeryStand(options: IsoBakeryStandOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(2856896684);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="12" rx="98" ry="26" fill="#000" opacity="0.35"/>');
parts.push(box(-58, -22, 0, 116, 44, 46, '#6d4c2f'));
parts.push(box(-64, -26, 46, 128, 52, 8, '#8d5a2b'));
for (let i = 0; i < 6; i++) parts.push(box(-62 + i * 21, -26, 54, 21, 38, 5, i % 2 ? '#fef3c7' : accent));
parts.push(box(-42, -14, 54, 28, 18, 12, '#c98a3d'));
parts.push(box(-4, -12, 54, 28, 18, 12, '#b97a34'));
parts.push(box(30, -16, 54, 26, 18, 12, '#d19a4a'));
for (let i = 0; i < 3; i++) parts.push(box(-40 + i * 36, -10, 66, 3, 12, 2, '#7c521e'));
parts.push(box(-22, -34, 62, 44, 6, 14, '#4e342e'));
parts.push(led(-14, -31, 70, accent, 2.2, 0));
parts.push(led(6, -31, 70, '#f87171', 2.2, 0.6));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#1a1410"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
