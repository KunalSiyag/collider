export interface IsoPizzaOvenOptions {
  accent?: string;
}
...
export function createIsoPizzaOven(options: IsoPizzaOvenOptions = {}): string {
  const { accent = '#f97316' } = options;
  const rand = rng(3045186682);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="66" rx="110" ry="28" fill="#000" opacity="0.35"/>');
parts.push(box(-64, -30, 0, 128, 60, 30, '#a8a29e'));
const dc = project(-20, 0, 78);
parts.push('<circle cx="' + dc[0].toFixed(1) + '" cy="' + dc[1].toFixed(1) + '" r="38" fill="#e7e5e4"/>');
parts.push('<circle cx="' + (dc[0] - 10).toFixed(1) + '" cy="' + (dc[1] - 8).toFixed(1) + '" r="30" fill="#f5f5f4"/>');
parts.push(box(-42, -32, 96, 20, 20, 26, '#78716c'));
parts.push(puff(-32, -22, 126, 8, 3, 0));
parts.push(puff(-30, -24, 134, 11, 3.6, 1.2));
parts.push('<path d="' + poly([[-38, 30.5, 34], [-6, 30.5, 34], [-6, 30.5, 58], [-38, 30.5, 58]]) + '" fill="#0a0a0a"/>');
parts.push('<path d="M-38 34 Q-22 22 -6 34" stroke="#0a0a0a" stroke-width="10" fill="none"/>');
const fire = project(-22, 20, 40);
parts.push('<ellipse cx="' + fire[0].toFixed(1) + '" cy="' + fire[1].toFixed(1) + '" rx="10" ry="5" fill="' + accent + '"><animate attributeName="fill" values="' + accent + ';#fbbf24;' + accent + '" dur="1.2s" repeatCount="indefinite"/></ellipse>');
parts.push(box(30, 24, 30, 52, 10, 3, '#c98a3d'));
const pz = project(50, 29, 33);
parts.push('<ellipse cx="' + pz[0].toFixed(1) + '" cy="' + pz[1].toFixed(1) + '" rx="13" ry="6.5" fill="#d97706"/><ellipse cx="' + pz[0].toFixed(1) + '" cy="' + pz[1].toFixed(1) + '" rx="9" ry="4.5" fill="#dc2626"/>');
parts.push(orb(46, 27, 34, 1.8, '#fef3c7'));
parts.push(orb(54, 31, 34, 1.8, '#fef3c7'));
for (let i = 0; i < 3; i++) parts.push(orb(-58 + i * 12, -40, 31, 5, '#fde68a'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#151009"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
