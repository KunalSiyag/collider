export interface IsoSculpturePlazaOptions {
  accent?: string;
}
...
export function createIsoSculpturePlaza(options: IsoSculpturePlazaOptions = {}): string {
  const { accent = '#fbbf24' } = options;
  const rand = rng(2820706271);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="80" rx="120" ry="30" fill="#27272a" opacity="0.8"/>');
for (let i = 0; i < 6; i++) parts.push('<path d="' + poly([[-100 + i * 34, -60, 1], [-100 + i * 34, 60, 1]]) + '" stroke="#3f3f46" stroke-width="1.5" fill="none"/>');
for (let j = 0; j < 4; j++) parts.push('<path d="' + poly([[-100, -60 + j * 30, 1], [70, -60 + j * 30, 1]]) + '" stroke="#3f3f46" stroke-width="1.5" fill="none"/>');
parts.push(cyl(0, 0, 0, 34, 8, '#52525b', 10));
parts.push(box(-18, -16, 8, 36, 32, 30, '#a8a29e'));
parts.push('<ellipse cx="' + project(0, 0, 46)[0].toFixed(1) + '" cy="' + project(0, 0, 46)[1].toFixed(1) + '" rx="26" ry="12" fill="#78716c"/>');
parts.push(orb(0, 0, 62, 17, accent));
parts.push(pyr(-10, -10, 82, 20, 20, 34, shade(accent, 0.85)));
parts.push('<path d="M' + project(0, 0, 116)[0].toFixed(1) + ' ' + project(0, 0, 116)[1].toFixed(1) + ' l0 -14" stroke="#d6d3d1" stroke-width="3" stroke-linecap="round"/>');
parts.push('<polygon points="-60,40 -160,10 -160,70" fill="' + accent + '" opacity="0.06"/><polygon points="60,40 160,10 160,70" fill="#8b5cf6" opacity="0.08"/>');
parts.push(orb(64, 44, 2, 3.5, '#a1a1aa'));
parts.push(orb(72, 52, 2, 3, '#a1a1aa'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#11101c"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
