export interface IsoTrafficLightOptions {
  accent?: string;
}
...
export function createIsoTrafficLight(options: IsoTrafficLightOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(2493027567);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="90" rx="115" ry="28" fill="#000" opacity="0.35"/>');
parts.push(box(-100, -20, 0, 200, 40, 4, '#27272a'));
for (let i = 0; i < 7; i++) parts.push(box(-92 + i * 28, -14, 4, 14, 28, 2, '#e7e5e4'));
parts.push(cyl(40, 20, 0, 5, 118, '#3f3f46', 8));
parts.push(box(28, 12, 118, 24, 18, 62, '#18181b'));
parts.push('<circle cx="' + project(40, 21, 168)[0].toFixed(1) + '" cy="' + project(40, 21, 168)[1].toFixed(1) + '" r="7" fill="#ef4444"><animate attributeName="opacity" values="1;0.15;0.15;0.15;1" keyTimes="0;0.32;0.33;0.98;1" dur="6s" repeatCount="indefinite"/></circle>');
parts.push('<circle cx="' + project(40, 21, 150)[0].toFixed(1) + '" cy="' + project(40, 21, 150)[1].toFixed(1) + '" r="7" fill="#fbbf24"><animate attributeName="opacity" values="0.15;1;0.15;0.15;0.15" keyTimes="0;0.33;0.65;0.99;1" dur="6s" repeatCount="indefinite"/></circle>');
parts.push('<circle cx="' + project(40, 21, 132)[0].toFixed(1) + '" cy="' + project(40, 21, 132)[1].toFixed(1) + '" r="7" fill="#4ade80"><animate attributeName="opacity" values="0.15;0.15;1;0.15;0.15" keyTimes="0;0.32;0.33;0.98;1" dur="6s" repeatCount="indefinite"/></circle>');
parts.push(box(34, 14, 118, 8, 14, 8, '#27272a'));
parts.push(orb(40, 20, 186, 3.5, '#a1a1aa'));
parts.push(orb(-30, 10, 6, 4, '#4d7c0f'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#0e0f14"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
