...
export function createIsoCamera(options: IsoCameraOptions = {}): string {
  const { accent = '#ef4444' } = options;
  const rand = rng(974119762);
  const parts: string[] = [];

parts.push('<ellipse cx="0" cy="80" rx="80" ry="20" fill="#000" opacity="0.35"/>');
parts.push('<path d="M-6 -60 L-52 82 M6 -60 L52 82 M0 -58 L-6 84" stroke="#3f3f46" stroke-width="5" stroke-linecap="round" fill="none"/>');
parts.push(box(-16, -12, 56, 32, 24, 12, '#27272a'));
parts.push(box(-30, -18, 68, 60, 36, 26, '#3f3f46'));
parts.push(orb(-12, 0, 100, 12, '#71717a'));
parts.push(orb(16, 8, 100, 12, '#71717a'));
parts.push(orb(-12, 0, 112, 5, '#27272a'));
parts.push(orb(16, 8, 112, 5, '#27272a'));
const lens = project(30, 18, 81);
parts.push('<circle cx="' + lens[0].toFixed(1) + '" cy="' + lens[1].toFixed(1) + '" r="9" fill="#0ea5e9"/>');
parts.push('<circle cx="' + (lens[0] - 3).toFixed(1) + '" cy="' + (lens[1] - 3).toFixed(1) + '" r="3" fill="#e0f2fe" opacity="0.7"/>');
parts.push(led(-24, 16, 86, accent, 1.2, 0));
parts.push(box(-8, -14, 94, 16, 8, 8, '#52525b'));

  return '<svg viewBox="0 0 320 320" xmlns="http://www.w3.org/2000/svg" aria-hidden="true">' +
    '<defs><linearGradient id="bg" x1="0" y1="0" x2="0" y2="1">' +
    '<stop offset="0" stop-color="#131118"/><stop offset="1" stop-color="#0b0b10"/>' +
    '</linearGradient></defs>' +
